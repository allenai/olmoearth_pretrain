...
from torch import Tensor, autocast, einsum, nn
...
class TopNGating(Module):
    ...
    def forward(self, x, routing_tokens, noise_gates=False, noise_mult=1.0):
        """Einstein notation:
...
class MoE(Module):
    ...
    def forward(self, x, routing_tokens, noise_gates=False, noise_mult=1.0):
        dispatch_tensor, combine_tensor, balance_loss, router_z_loss = self.gate(
            x, routing_tokens, noise_gates=noise_gates, noise_mult=noise_mult
        )
...
class SparseMoEBlockWithAttn(Module):
    ...
    def __init__(
        ...
        self.mlp = SparseMoEBlock(
            moe=MoE(dim=dim, num_experts=8, expert_hidden_mult=mlp_ratio)
        )
        self.ls2 = (
            LayerScale(dim, init_values=init_values) if init_values else nn.Identity()
        )
...
class MoEEncoder(Encoder):
    ...
    def forward(
        ...
        if token_exit_cfg is None or any(
            ...
            (patchified_tokens_and_masks, total_aux_losses) = self.apply_attn(
