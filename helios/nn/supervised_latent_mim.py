...
class SupervisedLatentMIM(nn.Module, DistributedMixins):
    """Supervised Latent MIM Style."""
    ...
    def forward(self, x: MaskedHeliosSample, patch_size: int) -> ModelOutput:
        ...
        latent, latent_projected_and_pooled, probe_outputs, spatial_tokens = (
            self.encoder(x, patch_size=patch_size)
        )
        reconstructed = None
...
@dataclass
class SupervisedLatentMIMConfig(Config):
    ...
    def build(self) -> "SupervisedLatentMIM":
