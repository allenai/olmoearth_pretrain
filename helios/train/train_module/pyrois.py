...
@dataclass
class PyroisTrainModuleConfig(HeliosTrainModuleConfig):
    ...
    ) -> "PyroisTrainModuleConfig":
        """Build the corresponding :class:`PyroisTrainModuleConfig`.
...
class PyroisTrainModule(HeliosTrainModule):
    """A :class:`TrainModule`.
    ...
    def model_forward(
        ...
        with self._model_forward_context():
            ...
            if self.mae_loss is not None:
                loss += self.mae_loss.compute(out["reconstructed"], batch)
            out["loss"] = loss
            return out
