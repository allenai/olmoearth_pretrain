...
class PerModalityPatchDiscriminationLoss(Loss):
    ...
    name = "PerModalityPatchDiscriminationLoss"
    ...
    def __init__(
        ...
        Args:
            ...
            sigma: the standard deviation of the Gaussian distribution
            pred2unit: whether to standardize the predictions using batch statistics
            min_tokens_per_modality: don't compute the loss if we have less than this many
                decoder tokens
        """
        self.tau = tau
        self.mu = mu
        self.sigma = sigma
        self.pred2unit = pred2unit
        self.min_tokens_per_modality = min_tokens_per_modality
    ...
    def _compute(
        ...
        if len(pred) < self.min_tokens_per_modality:
            return None
    ...
    def compute(
        ...
        loss = self._compute(
            *predictions.flatten_tokens_and_masks(),
            targets.flatten_tokens_and_masks()[0],
        )
        for modality in predictions.modalities:
            modality_loss = self._compute(
                *predictions.flatten_tokens_and_masks(modality),
                targets.flatten_tokens_and_masks(modality)[0],
            )
