...
class RunParams:
    ...
    def to_env_vars(self) -> Dict[str, str]:
        ...
        return {
            keys["checkpoint_path"]: os.path.join("/artifacts", constants.MODEL_SIZE_MAP[self.model_size]),
    ...
    def from_env_vars() -> 'RunParams':
        ...
        batch_sizes = [
            int(b)
            for b in os.getenv(keys["batch_sizes"], "1,").split(",")
            if b
        ]
        gpu_type = os.getenv(keys["gpu_type"], None)
        bf16 = True if os.getenv(keys["bf16"], "0") == "1" else False
        benchmark_interval_s = int(os.getenv(keys["benchmark_interval_s"], "180"))
        min_batches_per_interval = int(
            os.getenv(keys["min_batches_per_interval"], 10)
        )
