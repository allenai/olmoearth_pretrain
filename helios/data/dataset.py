...
def get_max_t_within_token_budget(
    modalities: list[str], h_w_p: int, max_tokens_per_instance: int
) -> int:
    """Find max t possible when subsetting.
...
def get_subset_dimensions(
    ...
    max_t = get_max_t_within_token_budget(
        modalities, sampled_hw_p, max_tokens_per_instance
    )
    sampled_hw = sampled_hw_p * patch_size
    ...
    return {
        ...
        "start_t": int(start_t),
        "max_t": int(max_t),
        "end_h": int(start_h + sampled_hw),
        "end_w": int(start_w + sampled_hw),
        "end_t": int(start_t + max_t),
    }
...
class HeliosDataset(Dataset):
    ...
    def __getitem__(self, args: GetItemArgs) -> tuple[int, HeliosSample]:
        ...
        dimensions = get_subset_dimensions(
            ...
            with h5py.File(
                ...
                for attribute, modality in h5file.items():
                    if (
                        ...
                        and attribute != "timestamps"
                        and attribute != "latlon"
                    ):
                        continue
                    assert modality is not None
