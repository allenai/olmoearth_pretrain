...
from olmo_earth.data.constants import ModalitySpec
...
def load_predefined_config() -> dict[str, dict[str, dict[str, float]]]:
    ...
    and max keys.
    """
    with (files("olmo_earth.data.norm_configs") / "predefined.json").open() as f:
        return json.load(f)
...
def load_computed_config() -> dict[str, dict]:
    ...
    and std keys.
    """
    with (files("olmo_earth.data.norm_configs") / "computed.json").open() as f:
        return json.load(f)
