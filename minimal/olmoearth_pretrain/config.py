...
class _StandaloneConfig:
    ...
    def _resolve_class(cls, class_name: str) -> type | None:
        ...
        Handles backward compatibility by mapping helios.* to olmoearth_pretrain.*
        """
        if "." not in class_name:
            return None
        
        ### HACK
        # Map helios.* to olmoearth_pretrain.* for backward compatibility
        if class_name.startswith("helios."):
            class_name = class_name.replace("helios.", "olmoearth_pretrain.", 1)
            # Special case: helios.nn.flexihelios -> olmoearth_pretrain.nn.flexi_vit
            if "flexihelios" in class_name:
