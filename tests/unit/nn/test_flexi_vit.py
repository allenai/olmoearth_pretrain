...
# ---------------------------------------------------------------------------
# FlexiPatchEmbed: linear vs conv equivalence
# ---------------------------------------------------------------------------


def _make_conv_and_linear_pair(
...
def test_linear_vs_conv_patch_embed_equivalence(
    ...
    embed_conv, embed_linear = _make_conv_and_linear_pair(
        modality, in_chans, embedding_size, patch_size_at_16
    )

    p_h, _ = embed_conv.patch_size
    # Spatial dims must be divisible by effective patch size
    H = W = max(p_h * 2, 16)
