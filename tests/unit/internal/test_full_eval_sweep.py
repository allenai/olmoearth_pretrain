...
class TestCreateLinearProbeArg:
    ...
    def test_basic_functionality(self):
        ...
        result = create_linear_probe_arg("eurosat", "probe_lr")
        expected = (
    ...
    def test_different_task_names(self):
        ...
        result = create_linear_probe_arg("cropharvest", "pooling_type")
        expected = "--trainer.callbacks.downstream_evaluator.tasks.cropharvest.pooling_type={arg}"
        assert result == expected

    def test_special_characters_in_names(self):
        """Test with special characters in task names."""
        result = create_linear_probe_arg("m-eurosat", "embedding_batch_size")
        expected = "--trainer.callbacks.downstream_evaluator.tasks.m-eurosat.embedding_batch_size={arg}"
        assert result == expected
...
class TestLoopThroughParams:
    ...
    def test_generates_all_combinations(self):
        ...
        params_list = list(loop_through_params())

        # Should generate len(LP_LRs) * len(Normalization_MODES) * len(pooling_types) combinations
        expected_count = len(LP_LRs) * len(Normalization_MODES) * len(pooling_types)
    ...
    def test_all_learning_rates_included(self):
...
class TestNoNormSweep:
    ...
    def test_generates_correct_combinations(self):
        ...
        params_list = list(no_norm_sweep())

        # Should generate len(pooling_types) * len(LP_LRs) combinations
        expected_count = len(pooling_types) * len(LP_LRs)
        assert len(params_list) == expected_count
...
class TestModelSpecificArgs:
    ...
    def test_get_dino_v3_args(self):
        """Test DinoV3 argument generation."""
        result = get_dino_v3_args()
    ...
    def test_get_panopticon_args(self):
        """Test Panopticon argument generation."""
        result = get_panopticon_args()
    ...
    def test_get_galileo_args_pretrained_normalizer(self):
        """Test Galileo args with pretrained normalizer."""
        result = get_galileo_args(pretrained_normalizer=True)
    ...
    def test_get_galileo_args_dataset_normalizer(self):
        """Test Galileo args with dataset normalizer."""
        result = get_galileo_args(pretrained_normalizer=False)
...
class TestBuildCommandsBasic:
    ...
    def test_build_commands_defaults_only(self, base_args):
    ...
    def test_build_commands_no_checkpoint_path(self, base_args):
    ...
    def test_build_commands_with_model_name(self, base_args):
...
class TestBuildCommandsModelTypes:
    ...
    def test_build_commands_dino_v3(self, base_args):
    ...
    def test_build_commands_panopticon(self, base_args):
    ...
    def test_build_commands_galileo(self, base_args):
...
class TestBuildCommandsSweep:
    ...
    def test_build_commands_full_sweep_default_model(self, base_args):
    ...
    def test_build_commands_sweep_dino_v3(self, base_args):
...
class TestBuildCommandsExecution:
    ...
    def test_build_commands_dry_run(self, base_args):
    ...
    def test_build_commands_local_cluster(self, base_args):
    ...
    def test_build_commands_remote_cluster(self, base_args):
...
class TestBuildCommandsEdgeCases:
    """Test edge cases and error conditions."""

    def test_build_commands_with_extra_cli(self, base_args):
        """Test build_commands with extra CLI arguments."""
        base_args.defaults_only = True
        extra_cli = ["--custom_arg=value", "--another_flag"]

        commands = build_commands(base_args, extra_cli)
    ...
    def test_build_commands_no_project_name(self, base_args):
...
class TestParametrizedTests:
    ...
    def test_model_specific_args_parametrized(
        self, model_type, expected_args, base_args
    ):
    ...
    def test_parameter_combinations(self, lr, norm_mode, pooling_type):
...
class TestIntegration:
    ...
    def test_full_workflow_minimal_args(self, minimal_args):
        """Test the complete workflow with minimal arguments."""
        commands = build_commands(minimal_args, [])
    ...
    def test_complex_workflow_with_all_options(self, base_args):
        ...
        with patch("helios.evals.datasets.configs.get_eval_mode") as mock_get_eval_mode:
            ...
            with patch(
                ...
                commands = build_commands(base_args, ["--extra", "args"])
