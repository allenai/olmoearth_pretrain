...
# ---------------------------------------------------------------------------
# Vectorized masked negatives vs looping masked negatives
# All tests use single modality so the original's all-modality flatten bug
# doesn't affect the comparison.
# ---------------------------------------------------------------------------


def test_masked_neg_vec_matches_looping_all_decoder() -> None:
    """Vec masked neg matches looping when all tokens are decoder tokens."""
    b, t_h, t_w, t, d = 5, 4, 4, 2, 8
    torch.manual_seed(42)
    ...
    loop_loss = ModalityPatchDiscriminationMaskedNegatives(tau=0.1).compute(
        preds, targets
    )
    vec_loss = ModalityPatchDiscriminationMaskedNegativesVec(tau=0.1).compute(
        preds, targets
    )
    assert torch.isclose(loop_loss, vec_loss, rtol=1e-4, atol=1e-6), (
        f"loop={loop_loss.item()}, vec={vec_loss.item()}"
    )


def test_masked_neg_vec_matches_looping_uneven_masks() -> None:
    """Vec matches looping with uneven decoder token counts per sample."""
    b, t_h, t_w, t, d = 5, 4, 4, 2, 8
    torch.manual_seed(123)
...
def test_masked_neg_vec_matches_looping_with_identical_targets() -> None:
    """Vec matches looping when some targets are identical (masking active)."""
    b, t_h, t_w, t, d = 4, 3, 3, 2, 8
    torch.manual_seed(42)

    target_s2 = torch.randn((b, t_h, t_w, t, d))
    # Make some tokens share identical targets to trigger masking
    target_s2[:, 0, 0, :, :] = target_s2[:, 0, 1, :, :]
    target_s2[:, 1, 0, :, :] = target_s2[:, 1, 1, :, :]
...
def test_masked_neg_vec_matches_looping_sample_skipped() -> None:
    """Vec matches looping when a sample is skipped (all targets identical)."""
    b, d = 4, 16
    t_h, t_w, t = 2, 2, 1
    torch.manual_seed(77)
    ...
    target_s2[0, :, :, :, :] = target_s2[0, 0, 0, 0, :].clone()
    # Sample 1 has distinct targets → should be computed
    # Samples 2, 3 normal
...
def test_masked_neg_vec_matches_looping_no_masking() -> None:
    """Vec matches looping when masking is disabled (threshold=2.0)."""
    b, t_h, t_w, t, d = 5, 4, 4, 2, 8
    ...
    # Threshold=2.0 means nothing exceeds it → no masking
    loop_loss = ModalityPatchDiscriminationMaskedNegatives(
        tau=0.1, same_target_threshold=2.0
    ).compute(preds, targets)
    vec_loss = ModalityPatchDiscriminationMaskedNegativesVec(
        tau=0.1, same_target_threshold=2.0
    ).compute(preds, targets)
    assert torch.isclose(loop_loss, vec_loss, rtol=1e-4, atol=1e-6), (
        f"loop={loop_loss.item()}, vec={vec_loss.item()}"
...
def test_masked_neg_vec_matches_looping_multiple_seeds() -> None:
    """Sweep 20 seeds to catch seed-dependent mismatches (single modality)."""
    b, t_h, t_w, t, d = 6, 4, 4, 2, 8
    ...
    for seed in range(20):
        ...
        target_s2 = torch.randn((b, t_h, t_w, t, d))
        # Sprinkle some identical targets
        if seed % 3 == 0:
            target_s2[:, 0, 0, :, :] = target_s2[:, 0, 1, :, :]
        ...
        assert torch.isclose(loop_loss, vec_loss, rtol=1e-4, atol=1e-6), (
            f"seed={seed}: loop={loop_loss.item()}, vec={vec_loss.item()}"
        )


def test_masked_neg_vec_gradients_match_looping() -> None:
    """Gradients from vec match looping for single modality."""
    b, t_h, t_w, t, d = 4, 3, 3, 2, 16
    ...
    for seed in [0, 7, 42]:
        ...
        # Looping path
        s2_loop = s2_data.clone().requires_grad_(True)
        preds_loop = TokensAndMasks(sentinel2_l2a=s2_loop, sentinel2_l2a_mask=s2_mask)
        targets_loop = TokensAndMasks(
            sentinel2_l2a=target_s2.clone(), sentinel2_l2a_mask=s2_mask
        )
        loss_loop = ModalityPatchDiscriminationMaskedNegatives(tau=0.1).compute(
            preds_loop, targets_loop
        )
        loss_loop.backward()

        # Vec path
        s2_vec = s2_data.clone().requires_grad_(True)
        preds_vec = TokensAndMasks(sentinel2_l2a=s2_vec, sentinel2_l2a_mask=s2_mask)
        targets_vec = TokensAndMasks(
            sentinel2_l2a=target_s2.clone(), sentinel2_l2a_mask=s2_mask
        )
        loss_vec = ModalityPatchDiscriminationMaskedNegativesVec(tau=0.1).compute(
            preds_vec, targets_vec
        )
        loss_vec.backward()

        assert torch.isclose(loss_loop, loss_vec, rtol=1e-4, atol=1e-6), (
            f"seed={seed}: loss loop={loss_loop.item()} vec={loss_vec.item()}"
        )
        assert torch.allclose(s2_loop.grad, s2_vec.grad, rtol=1e-4, atol=1e-6), (
            f"seed={seed}: grad max diff="
            f"{(s2_loop.grad - s2_vec.grad).abs().max().item()}"
        )


def test_masked_neg_vec_masking_lowers_loss() -> None:
    """Masking identical-target negatives should lower the loss."""
    b, t_h, t_w, t, d = 4, 3, 3, 2, 8
    torch.manual_seed(42)
    ...
    masked = ModalityPatchDiscriminationMaskedNegativesVec(tau=0.1).compute(
        preds, targets
    )
    unmasked = ModalityPatchDiscriminationMaskedNegativesVec(
        tau=0.1, same_target_threshold=2.0
    ).compute(preds, targets)

    assert masked < unmasked, (
        f"Masking should lower loss: masked={masked.item()}, unmasked={unmasked.item()}"
    )


def test_masked_neg_vec_zero_loss_no_decoder_tokens() -> None:
    """Loss should be 0 when no decoder tokens exist."""
    b, t_h, t_w, t, d = 4, 3, 3, 2, 8
    torch.manual_seed(88)

    preds = TokensAndMasks(
        sentinel2_l2a=torch.randn((b, t_h, t_w, t, d)),
        sentinel2_l2a_mask=torch.zeros((b, t_h, t_w, t), dtype=torch.long),
    )
    targets = TokensAndMasks(
        sentinel2_l2a=torch.randn((b, t_h, t_w, t, d)),
        sentinel2_l2a_mask=torch.zeros((b, t_h, t_w, t), dtype=torch.long),
    ...
    loss = ModalityPatchDiscriminationMaskedNegativesVec(tau=0.1).compute(
        preds, targets
    )
    assert loss.item() == 0.0


def test_masked_neg_vec_mask_negatives_for_modalities() -> None:
    """mask_negatives_for_modalities selectively applies masking per modality."""
    b, t_h, t_w, t, d = 4, 3, 3, 2, 8
    torch.manual_seed(42)

    # Both modalities have identical targets to trigger masking
    target_s2 = torch.randn((b, t_h, t_w, t, d))
    target_s2[:, 0, 0, :, :] = target_s2[:, 0, 1, :, :]
    target_wc = torch.randn((b, t_h, t_w, 1, d))
    target_wc[:, 0, 0, :, :] = target_wc[:, 0, 1, :, :]
    ...
    targets = TokensAndMasks(
        sentinel2_l2a=target_s2,
        sentinel2_l2a_mask=torch.ones((b, t_h, t_w, t)) * MaskValue.DECODER.value,
        worldcover=target_wc,
        worldcover_mask=torch.ones((b, t_h, t_w, 1)) * MaskValue.DECODER.value,
    )

    # Mask only worldcover negatives
    loss_wc_only = ModalityPatchDiscriminationMaskedNegativesVec(
        tau=0.1, mask_negatives_for_modalities=["worldcover"]
    ).compute(preds, targets)

    # Mask all modalities
    loss_all = ModalityPatchDiscriminationMaskedNegativesVec(
        tau=0.1, mask_negatives_for_modalities=None
    ).compute(preds, targets)

    # No masking at all
    loss_none = ModalityPatchDiscriminationMaskedNegativesVec(
        tau=0.1, same_target_threshold=2.0
    ).compute(preds, targets)

    # More masking → lower loss
    assert loss_all <= loss_wc_only <= loss_none
