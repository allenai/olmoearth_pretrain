...
def main() -> None:
    ...
    parser.add_argument(
        ...
        default=None,
        help=(
            "Override window_size on every selected task (default: the ws16 "
            "embedding-product convention). Must divide the stored sample size "
            "for tiled datasets (128 for pastis_rslearn)."
        ),
    )
    parser.add_argument(
        "--embedding_normalization",
        ...
        help=(
            ...
            "none (default, as the model emits them) or l2 (per-embedding, the "
            "deployed convention). Tags the run name with the arm."
        ),
    )
    parser.add_argument(
        "--quantization",
