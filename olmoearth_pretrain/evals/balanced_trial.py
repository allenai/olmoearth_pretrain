...
# reported metric; macro F1 and accuracy are disagreement detectors, and the two
# ranking metrics are threshold-free third opinions.
...
def run_balanced_trials(
    ...
    if draw_size >= pool_labels.numel():
        logger.warning(
            ...
            f"Lower cap/n_per_class. Skipping."
        )
        return BalancedTrialResult()
    ...
    for fold_idx in range(n_folds):
        ...
        # AEF's two-way structure: evaluate on everything in the pool not drawn.
        ...
        for name in classifier_names:
            ...
            eval_features_np = pool_embeddings_np[held_out.numpy()]
