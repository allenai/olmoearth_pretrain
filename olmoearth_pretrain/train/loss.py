...
class ModalityPatchDiscriminationMaskedNegatives(Loss):
    ...
    def compute(
        ...
        for all_preds, all_masks, all_targets, modality in zip(
            ...
            pred = all_preds[decoder_mask].unsqueeze(dim=0).float()
            target = all_targets[decoder_mask].unsqueeze(dim=0).float()
            bs, nt, _ = pred.shape
            if nt == 0:
                continue
...
class ModalityPatchDiscriminationMaskedNegativesVec(Loss):
    ...
    Fully parallelized (no per-sample loops). For each modality, sorts decoder
    tokens to the front, builds a batched similarity matrix, masks out columns
    whose target embedding is nearly identical to the row's target (cosine
    similarity > same_target_threshold), then computes cross-entropy.

    Samples where any decoder token has zero valid negatives are excluded from
    the loss average (matching the looping version's skip behavior).
    """
    ...
    def _compute_modality_loss_parallel(
        ...
        """Compute masked-negatives patch discrimination loss for one modality.
        ...
        Args:
            ...
            should_mask: Whether to mask same-target negatives for this modality

        Returns:
            Scalar loss for this modality.
        """
        batch_size, num_tokens, dim = all_preds.shape
        decoder_mask = all_masks == MaskValue.DECODER.value
        count = decoder_mask.sum(dim=-1)  # (batch,)

        # Sort decoder tokens to front, preserving relative order
        ...
        # Valid mask: first count[b] positions per sample
        range_tensor = torch.arange(num_tokens, device=count.device)
        valid_mask = range_tensor.unsqueeze(0) < count.unsqueeze(1)
        ...
        if should_mask:
            ...
            target_sim = torch.bmm(
                sorted_targets, sorted_targets.transpose(1, 2)
            )  # (batch, tokens, tokens)
            same_target = target_sim > self.same_target_threshold
    ...
    def compute(
        ...
        for all_preds, all_masks, all_targets, modality in zip(
            ...
            loss = self._compute_modality_loss_parallel(
                all_preds, all_masks, all_targets, should_mask
            )
            if self.modality_weights is not None:
                loss = loss * self.modality_weights.get(modality, 1.0)
            total_loss += loss
