"""Training module for LatentMIMLITE with contrastive loss."""
...
from olmoearth_pretrain.nn.latent_mim_lite import LatentMIMLITE
...
@dataclass
class LatentMIMLITEContrastiveTrainModuleConfig(OlmoEarthTrainModuleConfig):
    """Config for LatentMIMLITEContrastiveTrainModule.

    Like ContrastiveLatentMIMTrainModuleConfig but without EMA, reinit_targets,
    or MAE loss (no reconstructor in LatentMIMLITE).
    """
    ...
    ) -> "LatentMIMLITEContrastiveTrainModule":
        """Build the train module."""
        kwargs = self.prepare_kwargs()
        return LatentMIMLITEContrastiveTrainModule(
            model=model,
            device=device,
            **kwargs,
        )
...
class LatentMIMLITEContrastiveTrainModule(OlmoEarthTrainModule):
    """Training module for LatentMIMLITE.

    Same training loop as ContrastiveLatentMIMTrainModule but:
    - No EMA updates (target encoder has independent architecture)
    - No MAE/reconstructor loss
    - Target encoder is fully frozen
    """
    ...
    def train_batch(
        ...
        Args:
            batch: A (patch_size, MaskedOlmoEarthSample_a, MaskedOlmoEarthSample_b) tuple.
            dry_run: If True, skip metric recording and just run forward/backward.
        """
        # No EMA update -- target encoder is frozen with independent architecture
        self.model.train()
