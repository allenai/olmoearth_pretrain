...
from olmoearth_pretrain.evals.eval_wrapper import PrithviV2, Satlas, get_eval_wrapper
...
class DownstreamEvaluatorCallback(Callback):
    ...
    def _check_supported_modalities(self, evaluator: DownstreamEvaluator) -> bool:
        ...
        if isinstance(self.trainer.train_module.model, Satlas | PrithviV2):
            if len(task_instance_used_modalities) > 1:
                # satlas and Prithvi can only ingest one modality at a time
                return False
