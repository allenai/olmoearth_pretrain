    ...
    def apply_mask(
        ...
        for i in range(batch.batch_size):
            ...
            if missing_per_time is None:
                ...
                # only one not missing timestep, so lets use
                # random masking
                use_random_masking = True
            else:
                if np.random.random() < self.random_ratio:
                    ...
                    # sample the timesteps (from the not missing ones)
                    # which will be encoded and decoded. Bandsets without
                    # a temporal component will be randomly masked.
