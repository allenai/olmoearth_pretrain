...
turns the source's modality tensor into a target for that class.

For classification entries, the extractor produces a binary mask ``[B, H, W]``
with values in ``{0.0, 1.0}``.  For regression entries, the extractor produces
continuous values ``[B, H, W]``.
...
class ClassExtractor(Protocol):
    """Turn a modality tensor into a per-pixel target for a single class.

    The input is the raw modality tensor as it appears on an
    ``OlmoEarthSample`` for spatial single-timestep modalities — shape
    ``[B, H, W, 1, num_bands]`` — and the output is a target tensor of
    shape ``[B, H, W]``.

    For classification: values in ``{0.0, 1.0}`` (binary mask).
    For regression: continuous values (with MISSING_VALUE for missing pixels).

    Implementations must be deterministic and pure (no global state).
    """

    def __call__(self, tensor: torch.Tensor) -> torch.Tensor:
        """Extract a target for this class."""
        ...
...
@dataclass(frozen=True)
class NormalizedValueEqExtractor:
    """Match pixels by approximate equality to a normalized float class value.

    Like ``ValueEqExtractor`` but for normalized data where class IDs have been
    scaled to floats (e.g. WorldCover IDs 10..100 → 0.1..1.0 after min-max
    normalization).  Uses a tolerance-based comparison.
    """
    ...
    def __call__(self, tensor: torch.Tensor) -> torch.Tensor:
        """Extract a binary mask via approximate value equality."""
        if tensor.ndim != 5:
            raise ValueError(
                f"NormalizedValueEqExtractor expects [B, H, W, 1, C] tensor, "
                f"got shape {tuple(tensor.shape)}"
            )
        band = tensor[..., 0, 0]
        return ((band - self.class_value).abs() < self.tolerance).float()


@dataclass(frozen=True)
class RegressionExtractor:
    """Extract a continuous-valued band as a regression target.

    Returns the raw (normalized) values for a single band.  Missing pixels
    retain the sentinel ``MISSING_VALUE`` so that the loss function can mask
    them.
    """
    ...
    def __call__(self, tensor: torch.Tensor) -> torch.Tensor:
        """Return continuous values ``[B, H, W]`` from ``[B, H, W, 1, C]``."""
        if tensor.ndim != 5:
            raise ValueError(
                f"RegressionExtractor expects [B, H, W, 1, C] tensor, "
                f"got shape {tuple(tensor.shape)}"
            )
        return tensor[..., 0, self.band_index]


@dataclass(frozen=True)
class ClassEntry:
    """One classifiable class or regression target from a single label source."""
    ...
    """Callable producing a target from the source modality tensor."""

    task_type: str = "binary_classification"
    """One of ``"binary_classification"``, ``"classification"``, or ``"regression"``."""
    ...
    def is_regression(self) -> bool:
        """Whether this entry represents a regression target."""
