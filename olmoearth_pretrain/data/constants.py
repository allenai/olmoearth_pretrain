...
class TokenizationBandSet:
    ...
    bands: tuple[str, ...]  # Band names (must exist in modality's band_order)

    def __init__(self, bands: list[str] | tuple[str, ...]) -> None:
        """Initialize with bands as a tuple for hashability."""
        object.__setattr__(self, "bands", tuple(bands))

    def __hash__(self) -> int:
        """Hash this TokenizationBandSet."""
...
class ModalityTokenization:
    ...
    def compute_indices(self, base_modality: "ModalitySpec") -> list[list[int]]:
        ...
        Raises:
            KeyError: If a band name doesn't exist in the modality's band_order.
        """
        name_to_idx = {name: i for i, name in enumerate(base_modality.band_order)}
        return [[name_to_idx[b] for b in group.bands] for group in self.band_groups]
...
class TokenizationConfig:
    ...
    at training time. Modalities without overrides use their default
    bandset configuration from ModalitySpec.
    """
    ...
    def get_bandset_indices(self, modality_name: str) -> list[list[int]]:
        ...
        if modality_name in self.overrides:
            return self.overrides[modality_name].compute_indices(base_spec)
        return base_spec.bandsets_as_indices()
    ...
    def validate(self) -> None:
        ...
        Raises:
            ValueError: If any band name is invalid.
        """
