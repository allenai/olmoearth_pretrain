...
class Era5TaskDataset(Dataset):
    ...
    def __init__(
        ...
        # ERA5 requires z-score normalization (see _extract_era5): no-data cells
        # are mean-imputed to 0, valid only when the post-norm mean is 0.
...
def _build_task_dataset(
    ...
    # ERA5 normalization is applied here by the olmoearth Normalizer using
    # z-score stats from computed.json (era5l_day_10). The rslearn model.yaml
    # `Normalize` transform is stripped at dataset build, so this is the ONLY
