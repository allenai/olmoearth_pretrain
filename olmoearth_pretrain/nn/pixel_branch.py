...
Ported (``"thinconv"`` variant only) from the pixel-branch program
(``origin/favyen/20260819-ema``, itself a port of the dual-resolution encoder's conv
pixel branch). A stack of unconditioned ConvNeXt-style units runs on the DENSE pixel
grid of the time-series input modalities, independent of the coarse transformer trunk,
and its final per-pixel features initialize the pixel-resolution register grid of the
:class:`~olmoearth_pretrain.nn.flexi_vit.SpatialRegisterBottleneck`
(``register_pixel_grid``) through a zero-initialized projection.

The trunk-interleaved ``"conv"`` variant (FiLM from the coarse tokens, pooled residual
back into them) is deliberately NOT carried over: in the original program it was within
noise of ``"thinconv"`` on every eval while costing the fusion machinery, and the
question this branch asks on v1.3 is simply whether an independent high-resolution
register init beats initializing every pixel cell from one cloned latent.
...
initialization a model with this branch is EXACTLY the model without it -- the pixel
parameters receive gradients but contribute nothing at step 0.
...
class PixelPatchEmbed(nn.Module):
    ...
    def _add_positional_encodings(self, tokens: Tensor, patch_size: int) -> Tensor:
        ...
        by the register RoPE coordinates.
        """
...
class PlainConvStep(nn.Module):
    ...
    by a pointwise MLP, with no coupling to the coarse trunk. Affine-free LayerNorm:
    the gamma/beta gradient reduction over every pixel row is a measured cost at this
    scale, and the affine is redundant before the convolution / MLP that follow.
    """
...
class PixelRegisterBranch(nn.Module):
    ...
    2. :meth:`run_thin_steps` runs the whole conv stack once, independent of the
       trunk.
    ...
    def __init__(
        ...
        Args:
            ...
            branch_type: Only ``"thinconv"`` (standalone unconditioned stack) is
                implemented; the argument is kept so configs name the variant.
        ...
        self.steps = nn.ModuleList(
            [PlainConvStep(pixel_dim, kernel_size, mlp_ratio) for _ in range(num_steps)]
        )
    ...
    def zero_init(self) -> None:
        ...
        Call AFTER any blanket weight init. At step 0 the register grid is exactly
        what it would be without the branch, while gradients still reach every pixel
        parameter through the zeroed projection (its weight gradient is non-zero, and
        once the weight moves off zero the conv stack receives gradient too).
        """
