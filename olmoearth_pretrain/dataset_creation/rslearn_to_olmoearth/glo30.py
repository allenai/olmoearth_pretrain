"""Post-process ingested Copernicus GLO-30 DSM data into the OlmoEarth Pretrain dataset."""
...
# The GLO-30 DEM is static; elevation was acquired by TanDEM-X between 2011 and 2015.
START_TIME = datetime(2011, 1, 1, tzinfo=UTC)
END_TIME = datetime(2015, 1, 1, tzinfo=UTC)

# Layer name in the input rslearn dataset.
LAYER_NAME = "glo30"


def convert_glo30(window: Window, olmoearth_path: UPath) -> None:
    """Add GLO-30 DSM data for this window to the OlmoEarth Pretrain dataset.
    ...
    assert len(Modality.GLO30.band_sets) == 1
    band_set = Modality.GLO30.band_sets[0]
    ...
    metadata_fname = get_modality_temp_meta_fname(
        olmoearth_path, Modality.GLO30, TimeSpan.STATIC, window.name
    )
...
if __name__ == "__main__":
    ...
    outputs = star_imap_unordered(p, convert_glo30, jobs)
