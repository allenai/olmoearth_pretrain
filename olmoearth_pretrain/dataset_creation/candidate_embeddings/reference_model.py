"""Shared reference-model infrastructure for candidate embedding scoring.

Provides:

- Embedding I/O (``load_embeddings``, ``ensure_index_rows``)
- PCA fitting / projection
- Spherical k-means fitting / assignment
- Residual computation
- Reference artifact persistence and loading
- Score parquet writing helpers
- JSON summary helper

The ``fit-reference`` CLI subcommand lives here because it is tightly
coupled to the artifact layout.  Individual scoring strategies
(novelty, acquisition, ...) import the functions they need from this
module.
"""
...
# ---------------------------------------------------------------------------
# Reference kNN self-scores & thresholds
# ---------------------------------------------------------------------------
...
def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    """Build the CLI parser for fit-reference."""
    parser = argparse.ArgumentParser(
        description="Fit frozen PCA / k-means reference artifacts for candidate scoring.",
    ...
    parser.add_argument("--k", type=int, default=15, help="Number of parent clusters")
    parser.add_argument(
