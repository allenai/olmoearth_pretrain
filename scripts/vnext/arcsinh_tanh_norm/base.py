"""arcsinh_tanh normalization baseline: v1.2 config with arcsinh_tanh input norm.

This is a self-contained baseline for experimenting with the ``arcsinh_tanh``
normalization strategy. It mirrors the v1.2 model/training setup (v1.1 hidden
patch-embed projection + mixed 3D RoPE) and changes exactly one thing: the
dataset uses ``norm_strategy="arcsinh_tanh"`` instead of the default linear
mean/std normalization.

The ``arcsinh_tanh`` strategy applies a per-band variance-stabilizing transform
(``arcsinh`` for right-skewed optical bands, identity for already-Gaussian bands
such as Sentinel-1 dB and Landsat thermal) -> z-score in the transformed space
-> ``tanh(gain * z)``, mapping the bulk of the distribution across most of
(-1, 1) while saturating outliers (e.g. clouds) near +/-1. Statistics live in
``olmoearth_pretrain/data/norm_configs/computed_arcsinh_tanh.json``.

Downstream evals that use ``norm_stats_from_pretrained=True`` automatically pick
up this strategy (the training dataset's ``norm_strategy`` / ``tanh_gain`` are
propagated to the downstream evaluator in
``olmoearth_pretrain.internal.experiment.build_config``).

This directory is intended to be independent; nothing here references the v1.2
scripts.
"""
...
# Normalization: per-band arcsinh/identity VST -> z-score -> tanh(gain * z).
...
def build_dataset_config(common: CommonComponents) -> OlmoEarthDatasetConfig:
    ...
    Uses the ``arcsinh_tanh`` normalization strategy (per-band arcsinh/identity
    VST -> z-score -> tanh) instead of the default linear mean/std normalization.
    """
...
def build_trainer_config(common: CommonComponents) -> TrainerConfig:
    ...
    WANDB_PROJECT = "2026_07_08_arcsinh_tanh_norm"
...
def build_size_model_config(
    ...
    """Build a model config (v1.2 mixed 3D RoPE) for the given size preset."""
...
def build_model_config(common: CommonComponents) -> LatentMIMConfig:
    """Build the default base model config (base encoder, decoder depth 4)."""
