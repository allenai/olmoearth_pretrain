"""Per-modality contrastive experiment.

Same as base_band_dropout but with per_modality_contrastive=True:
computes a separate InfoNCE loss per encode modality (S2, S1, Landsat)
instead of on the globally pooled vector. Ensures each modality's
encoder pathway gets direct contrastive gradient signal.

In-loop evals: eurosat KNN, pastis S2/S1 LP, forestnet KNN, mados LP
with embedding diagnostics enabled on all tasks.
"""
...
S2_SINGLE_BANDSET = ModalityTokenization(
    band_groups=[["B02", "B03", "B04", "B08", "B05", "B06", "B07", "B8A", "B11", "B12", "B01", "B09"]]
)
...
def build_trainer_config(common: CommonComponents) -> TrainerConfig:
    ...
    WANDB_PROJECT = "2026_04_per_modality_contrastive"
    ...
    EVAL_TASKS = {
        "m_eurosat": DownstreamTaskConfig(
            ...
            run_embedding_diagnostics=True,
        ),
