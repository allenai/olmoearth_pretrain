...
# ============================================================
# Experiment 18: single bandset (all 12 bands) + SpectralMixer + random band dropout
...
def build_common_exp18(
    ...
    """Build common components for exp18."""
    return _build_common(script, cmd, run_name, cluster, overrides)
...
def build_train_module_exp18(
    ...
    """Build train module for exp18."""
    return _build_train_module(common, "modality_cross_random")


def build_model_exp18(common: CommonComponents) -> LatentMIMConfig:
    """Build model for exp18: single bandset + SpectralMixer + random band dropout."""
    model_size = MODEL_SIZE_ARGS["base_shallow_decoder"]
    encoder_config = EncoderConfig(
        embedding_size=model_size["encoder_embedding_size"],
        num_heads=model_size["encoder_num_heads"],
...
def build_dataloader_exp18(common: CommonComponents) -> OlmoEarthDataLoaderConfig:
    """Build dataloader for exp18."""
    return _build_dataloader(common, "modality_cross_random")


# ============================================================
# Experiment 19: single bandset (all 12 bands) + SpectralMixer + random band dropout
...
def build_common_exp19(
    ...
    """Build common components for exp19."""
    return _build_common(script, cmd, run_name, cluster, overrides)
...
def build_train_module_exp19(
    ...
    """Build train module for exp19."""
    return _build_train_module(common, "random_with_decode")


def build_model_exp19(common: CommonComponents) -> LatentMIMConfig:
    """Build model for exp19: single bandset + SpectralMixer + random band dropout."""
    model_size = MODEL_SIZE_ARGS["base_shallow_decoder"]
    encoder_config = EncoderConfig(
        embedding_size=model_size["encoder_embedding_size"],
        num_heads=model_size["encoder_num_heads"],
...
def build_dataloader_exp19(common: CommonComponents) -> OlmoEarthDataLoaderConfig:
    """Build dataloader for exp19."""
    return _build_dataloader(common, "random_with_decode")


# ============================================================
# Experiment 20: single bandset (all 12 bands) + SpectralAttention (d=64) + random band dropout
#                + random_with_decode masking
# ============================================================
...
def build_common_exp20(
    ...
    """Build common components for exp20."""
    return _build_common(script, cmd, run_name, cluster, overrides)
...
def build_train_module_exp20(
    ...
    """Build train module for exp20."""
    return _build_train_module(common, "random_with_decode")


def build_model_exp20(common: CommonComponents) -> LatentMIMConfig:
    """Build model for exp20: single bandset + SpectralAttention d=64 + random band dropout."""
    model_size = MODEL_SIZE_ARGS["base_shallow_decoder"]
    encoder_config = EncoderConfig(
        embedding_size=model_size["encoder_embedding_size"],
        num_heads=model_size["encoder_num_heads"],
...
def build_dataloader_exp20(common: CommonComponents) -> OlmoEarthDataLoaderConfig:
    """Build dataloader for exp20."""
    return _build_dataloader(common, "random_with_decode")


# ============================================================
# Experiment 21: single bandset (all 12 bands) + SpectralAttention (d=128) + random band dropout
#                + random_with_decode masking
# ============================================================
...
def build_common_exp21(
    ...
    """Build common components for exp21."""
    return _build_common(script, cmd, run_name, cluster, overrides)
...
def build_train_module_exp21(
    ...
    """Build train module for exp21."""
    return _build_train_module(common, "random_with_decode")


def build_model_exp21(common: CommonComponents) -> LatentMIMConfig:
    """Build model for exp21: single bandset + SpectralAttention d=128 + random band dropout."""
    model_size = MODEL_SIZE_ARGS["base_shallow_decoder"]
    encoder_config = EncoderConfig(
        embedding_size=model_size["encoder_embedding_size"],
        num_heads=model_size["encoder_num_heads"],
...
# ============================================================
# Experiment 23: Exp21 model (SpectralAttention d=128) + Exp22 speedups
#                + orthogonal target projection init
# ============================================================
...
def build_common_exp23(
    ...
    """Build common components for exp23."""
    return _build_common(script, cmd, run_name, cluster, overrides)
...
def build_train_module_exp23(
    ...
    """Build train module for exp23: fused AdamW + compile + vec masked neg loss."""
    return ContrastiveLatentMIMTrainModuleConfig(
        optim_config=AdamWConfig(lr=0.0001, weight_decay=0.02, fused=True),
        rank_microbatch_size=32,
        masking_config=_masking_config(
...
def build_model_exp23(common: CommonComponents) -> LatentMIMConfig:
    """Build model for exp23: exp21 model + orthogonal target projection init."""
    model_size = MODEL_SIZE_ARGS["base_shallow_decoder"]
    encoder_config = EncoderConfig(
        embedding_size=model_size["encoder_embedding_size"],
        num_heads=model_size["encoder_num_heads"],
...
EXPERIMENTS = {
    ...
    "single_bandset_all12_spectral_attn_d64_random_band_dropout_random_decode_masked_neg": (
        build_common_exp20,
        build_model_exp20,
        build_train_module_exp20,
        build_dataloader_exp20,
    ),
    "single_bandset_all12_spectral_attn_d128_random_band_dropout_random_decode_masked_neg": (
        build_common_exp21,
        build_model_exp21,
    ...
    "single_bandset_all12_spectral_attn_d128_orthogonal_target_random_decode_masked_neg": (
        build_common_exp23,
        build_model_exp23,
