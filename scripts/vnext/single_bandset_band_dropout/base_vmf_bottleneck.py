"""VMF bottleneck experiment: output_embedding_size + L2-normalization + VMF noise.

Matches the setup from yawenz/20260412_smaller_embedding (random_time_with_decode masking,
masked-negatives loss, InfoNCE, no S1 band dropout, same evals) but adds a VMF bottleneck
after the linear output projection.

Key differences from the linear-only `output_embedding_size` baseline:
- L2-normalizes projected tokens onto the unit hypersphere
- During training, adds VMF noise with concentration kappa (smooths the embedding manifold)
- Optional batch uniformity regularizer prevents collapse on the sphere

Configurable via CLI:
    --model.encoder_config.output_embedding_size=128
    --model.encoder_config.vmf_bottleneck_kappa=8000
    --train_module.regularizer_config.loss_config.type=batch_uniformity
    --train_module.regularizer_config.loss_config.weight=0.05
"""
...
# Default VMF parameters — override via CLI
...
def build_trainer_config(common: CommonComponents) -> TrainerConfig:
    ...
    EVAL_TASKS = {
        ...
        # --- Embedding diagnostics (effective rank, uniformity, cosine stats) ---
        "embed_diag_eurosat": DownstreamTaskConfig(
            dataset="m-eurosat",
            embedding_batch_size=128,
            num_workers=0,
            pooling_type=PoolingType.MEAN,
        ...
        "embed_diag_so2sat": DownstreamTaskConfig(
            dataset="m-so2sat",
            embedding_batch_size=128,
            num_workers=4,
            pooling_type=PoolingType.MEAN,
        ...
        "embed_diag_pastis_s2": DownstreamTaskConfig(
            dataset="pastis",
            embedding_batch_size=32,
            num_workers=2,
            pooling_type=PoolingType.MEAN,
...
def build_model_config(common: CommonComponents) -> LatentMIMConfig:
    ...
    encoder_config = EncoderConfig(
        ...
        vmf_bottleneck_kappa=VMF_KAPPA,
    )
