"""Base script for single bandset + spectral attention + random band dropout (no S1) + random time with decode masking.

Same as base_band_dropout_no_s1_drop_random_time.py but with SpectralAttention (d=128, 2 heads)
applied to satellite modalities (S2, S1, Landsat) before patch embedding.
"""
...
def build_model_config(common: CommonComponents) -> LatentMIMConfig:
    ...
    encoder_config = EncoderConfig(
        ...
        spectral_attention_num_heads=2,
        spectral_attention_modalities=SATELLITE_MODALITIES,
    )
