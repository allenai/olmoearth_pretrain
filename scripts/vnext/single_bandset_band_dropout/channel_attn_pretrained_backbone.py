"""Channel-attention patch embed initialised from a pretrained backbone checkpoint.

Same model architecture as channel_attn_patch_embed.py, but designed to load a
pretrained checkpoint whose patch embedding is the old linear projection.
Matching weights (ViT blocks, composite encodings, norm, projection head,
decoder, target encoder) are loaded; the new ChannelAttentionPatchEmbed layers
are randomly initialised since they have no counterpart in the checkpoint.

Two modes controlled by the REINIT_DECODER env-var:
  REINIT_DECODER=false (default) -- decoder keeps pretrained weights
  REINIT_DECODER=true            -- decoder weights are reinitialised

The checkpoint path is intentionally NOT hard-coded; always pass it via the
CLI override --trainer.load_path=<path>.  For resumed (preempted) runs the
trainer transparently loads from save_folder first, so load_path is only used
...
REINIT_DECODER = os.environ.get("REINIT_DECODER", "false").lower() == "true"
...
def build_trainer_config(common: CommonComponents) -> TrainerConfig:
    ...
    WANDB_PROJECT = "2026_05_15_channel_attn_pretrained_backbone"
    ...
    if REINIT_DECODER:
        trainer_config = trainer_config.with_callback(
            "reinit_decoder", ReinitDecoderCallback()
        )
