...
Like base_band_dropout_no_s1_drop_random_time.py, but with:
- EMA target encoder (0.996 -> 1.0)
- Full target encoder (exit depth 12) for sentinel1, sentinel2, and landsat
"""
...
# Full target encoder depth for these modalities.
FULL_TARGET_MODALITIES = [
    Modality.SENTINEL1.name,
    Modality.SENTINEL2_L2A.name,
    Modality.LANDSAT.name,
]

ENCODER_DEPTH = MODEL_SIZE_ARGS["base_shallow_decoder"]["encoder_depth"]
...
def build_train_module_config(
    ...
    token_exit_cfg = {
        modality: ENCODER_DEPTH if modality in FULL_TARGET_MODALITIES else 0
        for modality in common.training_modalities
    }
