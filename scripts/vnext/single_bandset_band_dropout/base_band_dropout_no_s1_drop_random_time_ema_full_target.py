"""Single bandset + random band dropout on S2/Landsat only (no S1 dropout) + random_time_with_decode + EMA.

Reconstructed from the saved run config for
``single_bandset_no_s1_drop_random_time_updatedv2_dropout_0.2_fullema`` (gabrielt), for loading
those checkpoints in eval / resume. Differences vs ``base_band_dropout.py``:

- Masking: ``random_time_with_decode`` (``random_ratio=0.5``).
- Band dropout: fixed rate 0.2 on ``sentinel2_l2a`` and ``landsat`` only (not Sentinel-1).
- Train module: EMA (0.996, 1.0), contrastive InfoNCE weight 0.05, ``rank_microbatch_size=64``,
  token exits at full depth for S1/S2/Landsat spectral streams.
"""
...
# Band dropout on spectral stacks only; Sentinel-1 excluded ("no S1 drop").
...
def _token_exit_cfg_full_ema(common: CommonComponents) -> dict[str, int]:
    depth = MODEL_SIZE_ARGS["base_shallow_decoder"]["encoder_depth"]
    return {
...
def build_trainer_config(common: CommonComponents) -> TrainerConfig:
    """Build the trainer config for an experiment (matches saved in-loop eval tasks)."""
