"""Linear patch embed + constant band dropout from a pretrained checkpoint (reinit decoder).

Same model architecture as base_band_dropout.py (standard linear patch projection)
with constant band dropout rate 0.3 from step 0 (no curriculum).  Loads all weights
from a pretrained checkpoint.  Decoder weights are reinitialised after loading so
only the encoder backbone is pretrained.

Eval runs at step 0 (eval_on_startup=True) for baseline comparison.
