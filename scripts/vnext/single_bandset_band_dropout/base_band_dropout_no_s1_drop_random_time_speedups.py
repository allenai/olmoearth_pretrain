"""Gabi's base config with training speedups applied.

Identical to base_band_dropout_no_s1_drop_random_time.py except:
- Vectorized masked-negatives loss (modality_patch_discrimination_masked_negatives_vec)
- Linear patch embedding (use_linear_patch_embed=True, already the default)
- torch.compile enabled (compile_model=True)

Usage:
    python scripts/vnext/single_bandset_band_dropout/base_band_dropout_no_s1_drop_random_time_speedups.py dry_run <RUN_NAME> local
    python scripts/vnext/single_bandset_band_dropout/base_band_dropout_no_s1_drop_random_time_speedups.py launch <RUN_NAME> <CLUSTER>
"""
...
def build_train_module_config(
    ...
    return ContrastiveLatentMIMTrainModuleConfig(
        compile_model=True,  # SPEEDUP: torch.compile
        optim_config=AdamWConfig(lr=0.0001, weight_decay=0.02, fused=False),
        rank_microbatch_size=64,
        masking_config=_masking_config(common.tokenization_config),
        loss_config=LossConfig(
            loss_config={
                "type": "modality_patch_discrimination_masked_negatives_vec",  # SPEEDUP: vectorized loss
                "tau": 0.1,
