"""Base script for single bandset + random band dropout (no S1) + random time with decode masking + static temporal encoding.

Same as base_band_dropout_no_s1_drop_random_time.py but with
timestamp_encoding_mode="static_temporal" so:
- Temporal: multi-frequency sinusoidal (get_static_temporal_encoding) in legacy 2-quarter slot
- Spatial: legacy 2D sincos pos encoding (not per-token lat/lon)
- No latlon_dropout / spatial_dim_fraction / temporal_dim_fraction needed
"""
