"""Linear patch embed initialised from a pretrained checkpoint (reinit decoder).

Same model architecture as base_band_dropout.py (standard linear patch projection),
but loads all weights from a pretrained checkpoint.  If the checkpoint architecture
differs, mismatched keys are left at their random init while all matching weights
are loaded.  Decoder weights are reinitialised after loading so only the encoder
backbone is pretrained.

See linear_patch_embed_pretrained_keep_decoder.py for the variant that keeps the
