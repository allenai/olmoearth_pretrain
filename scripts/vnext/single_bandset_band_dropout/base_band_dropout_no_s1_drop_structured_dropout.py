"""Base script for single bandset + structured band dropout (no S1) + random time with decode masking + masked-negatives loss.

- Single bandset S2 (all 12 bands) / Landsat (all 11 bands)
- Structured band dropout (rate ~ Uniform(0, 0.3), deterministic count, shared across batch)
  on S2 and Landsat only (no S1 dropout)
