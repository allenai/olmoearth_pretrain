"""Single bandset + channel-attention patch embedding (large attn_dim) + curriculum band dropout.

Same as channel_attn_patch_embed.py but with a larger cross-attention bottleneck
(attn_dim=1536, 2x the encoder embedding_size) for richer per-band spatial
representation before fusion.

Band dropout follows the same 3-stage curriculum used by channel_attn_patch_embed.py
(see `BandDropoutCurriculumCallback`); `band_dropout_rate=0.0` in EncoderConfig is
the initial value, and the callback drives the actual rate per-step.
