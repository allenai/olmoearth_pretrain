"""Ablation: patch-level var+cov on satellite modalities only (no pooled VICReg).

Matches ``vicreg_sweep_half.py`` hyperparameters for a clean ablation against
``vicreg_half_v1``, but replaces the pooled VICRegViews term with a
patch-level variance + covariance regularizer computed on encoder-visible
student latent tokens for S2 / S1 / Landsat only.

Weights mirror the ``half`` VICReg run: var=0.05, cov=0.005, gamma=1.0.
"""
    ...
    return ContrastiveLatentMIMTrainModuleConfig(
        ...
        patch_varcov_config=LossConfig(
            loss_config={
                ...
                "cov_weight": 0.005,
                "gamma": 1.0,
                "modalities": PATCH_VARCOV_MODALITIES,
            }
        ),
...
def build_trainer_config(common: CommonComponents) -> TrainerConfig:
    ...
    WANDB_PROJECT = "2026_04_20_vicreg_patch_ablations"
