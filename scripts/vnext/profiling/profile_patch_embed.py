"""Profile linear vs conv patch embedding on the near-final training config.

Usage:
    # Dry run
    python scripts/vnext/profiling/profile_patch_embed.py dry_run profile_linear local

    # Linear patch embed (default, use_linear_patch_embed=True)
    python scripts/vnext/profiling/profile_patch_embed.py <CMD> profile_linear <CLUSTER>

    # Conv patch embed
    python scripts/vnext/profiling/profile_patch_embed.py <CMD> profile_conv <CLUSTER> \
        --model.encoder_config.use_linear_patch_embed=False

Based on gabi/masking-v2 base_band_dropout_no_s1_drop_random_time.py with:
- ProfilerCallback enabled (skip_first=5, wait=2, warmup=3, active=5)
- Short max_duration (100 steps)
- Evals and checkpointing removed to isolate model throughput
"""
...
def build_trainer_config(common: CommonComponents) -> TrainerConfig:
    wandb_callback = OlmoEarthWandBCallback(
        ...
        project="2026_04_15_profile_patch_embed",
    ...
    return (
        ...
        .with_callback("profiler", profiler_callback)
    )
