...
onto the coarse token. Runs after every 3rd coarse block; pixels never attend across
...
~1.75x on a heavy large-patch mix; single-GPU benchmark, ``benchmark_pixel_branch.py``
variant ``window_d64_every3``), vs ~8x for the original joint pixel branch. This is
the stronger-cadence sibling of ``window_d64_e4.py`` (4 fusion steps instead of 3).
"""
