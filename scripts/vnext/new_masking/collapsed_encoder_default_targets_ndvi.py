"""Collapsed bandsets for encoder, default bandsets for targets, with NDVI.

Same as collapsed_encoder_default_targets.py but adds NDVI as a decode-only
modality. NDVI is computed from Sentinel-2 L2A bands B04 (Red) and B08 (NIR)
in the dataset before normalization.

Change from collapsed_encoder_default_targets:
- Adds NDVI to training modalities
- NDVI is decode-only (never seen by the encoder, only predicted by the decoder)
- NDVI uses a single token (1 band) in both collapsed and default tokenization
"""
...
def get_masking_config(common: CommonComponents):
    ...
    # Add NDVI to the only_decode_modalities list
...
def build_model_config(common: CommonComponents) -> LatentMIMConfig:
    """Build model config with collapsed encoder, default-tokenization targets, and NDVI."""
