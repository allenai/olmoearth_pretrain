"""Smaller-embedding base: v1.2 (mixed 3D RoPE) with a high-dim loss target.

Self-contained copy of ``scripts/official/v1_2/base.py`` specialized for the
smaller-embedding study. The functional deltas vs v1.2 base are:

* Only 4 in-loop evals (m-eurosat, m_so2sat, mados, pastis).
* W&B project ``2026_04_12_smaller_embedding``.
* ``build_smaller_embedding_model_config`` wires the encoder to emit a compact
  deliverable embedding while the patch-discrimination loss (and its frozen
  exit-0 target) run at ``target_embedding_size`` (768 by default):
    - Approach 1 (base): full 768 encoder, bottleneck to ``output_embedding_size``
      in {384, 192, 96}; target is the raw 768 patch-embedding.
    - Approach 2 (tiny/nano): native small encoder, no bottleneck; target is the
      patch-embedding expanded up to 768 by a frozen MLP.

Size variants live in the sibling scripts (base_dim*.py, tiny_up768.py, ...).
"""
...
# projection. See v1.2 base.py for details.
PATCH_EMBED_HIDDEN_SIZES: list[int] = [64]

# The dimension the patch-discrimination loss (and its frozen target) run at.
LOSS_EMBEDDING_SIZE = 768

# Mixed 3D RoPE knobs -- the winning trope_mixed_tscale_months config.
...
def build_smaller_embedding_model_config(
    common: CommonComponents,
    size_name: str,
    patch_embed_hidden_sizes: list[int],
    output_embedding_size: int | None = None,
    target_embedding_size: int = LOSS_EMBEDDING_SIZE,
    target_expander_num_layers: int = 2,
) -> LatentMIMConfig:
    """Build a smaller-embedding model config.

    The encoder emits a compact deliverable embedding (``output_embedding_size``
    if set, else the native encoder width); the decoder predicts, and the frozen
    exit-0 target operates, at ``target_embedding_size``.

    Args:
        common: Common components (modalities, tokenization) for the run.
        size_name: Key into ``MODEL_SIZE_ARGS`` (encoder width, depth, heads).
        patch_embed_hidden_sizes: Per-pixel patch-embed MLP widths.
        output_embedding_size: Deliverable embedding dim the encoder emits and
            the decoder consumes. ``None`` keeps the native encoder width
            (Approach 2). Set to a bottleneck (Approach 1) for the base encoder.
        target_embedding_size: Dimension the loss and its frozen target run at.
        target_expander_num_layers: Layers in the frozen target expander MLP,
            used only when the patch-embed width differs from the target width.
    """
    model_size = MODEL_SIZE_ARGS[size_name]
    encoder_width = model_size["encoder_embedding_size"]
    # Dim the decoder consumes = whatever the encoder emits.
    decoder_input_size = output_embedding_size or encoder_width
    ...
    decoder_config = PredictorConfig(
        encoder_embedding_size=decoder_input_size,
    ...
    return LatentMIMConfig(
        ...
        target_expander_num_layers=target_expander_num_layers,
    )
...
if __name__ == "__main__":
    main(
        ...
        model_config_builder=lambda common: build_smaller_embedding_model_config(
            common,
            "base_shallow_decoder",
            PATCH_EMBED_HIDDEN_SIZES,
            output_embedding_size=192,
        ),
