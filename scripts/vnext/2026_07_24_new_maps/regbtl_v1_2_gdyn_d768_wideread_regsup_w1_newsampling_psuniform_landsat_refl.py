"""d768 wideread + regsup at base_weight 1.0 (newmaps) on LANDSAT REFLECTANCE h5.

Exact twin of ``regbtl_v1_2_gdyn_d768_wideread_regsup_w0p1_newsampling_psuniform_landsat_refl``
-- same d768 full-width Perceiver register bottleneck, same 1fwd + fused AdamW train
module, same decorrelated newsampling at uniform patch sizes, same reflectance h5 and
reflectance-scale Landsat norm stats -- with ONE change:
...
With the d128 w1 twin this closes a width x supervision-strength 2x2 on
reflectance: whether the map supervision wants to pull harder may well depend on
how much room the register grid has to store what it is being asked to predict,
which is exactly what the two widths differ in.
"""
...
MODULE_PATH = (
    ...
    "regbtl_v1_2_gdyn_d768_wideread_regsup_w1_newsampling_psuniform_landsat_refl.py"
)
...
def build_model_config(common: CommonComponents) -> LatentMIMConfig:
    """d768 wideread + new-maps register-grid supervision at w1 (base_weight 1.0)."""
