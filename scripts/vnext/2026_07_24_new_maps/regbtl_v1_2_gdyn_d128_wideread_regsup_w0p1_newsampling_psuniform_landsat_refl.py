"""d128 wideread + regsup (newmaps) on LANDSAT REFLECTANCE h5.

Exact twin of ``regbtl_v1_2_gdyn_d128_wideread_regsup_w0p1_newsampling_psuniform``
(Perceiver register bottleneck at register_dim=128, wideread/no-compression reads at
encoder width, 1fwd + fused AdamW, decorrelated newsampling at uniform patch sizes,
register-grid supervision at base_weight 0.1) -- the ONLY differences are the dataset:
...
This is the fourth cell of the width x radiometry 2x2, whose other three
(d768 DN, d768 reflectance, d128 DN) are already trained. Having all four is what
lets radiometry be separated from run-to-run drift: a same-data width change moved
the Landsat patch embedding as far as the radiometry change did, so the d768-only
A/B could not tell the two apart on its own.

register_dim 128 is the compressed bottleneck (storage width 128, reads at encoder
width 768); the d768 reflectance twin is the full-width register grid.
"""
...
MODULE_PATH = (
    ...
    "regbtl_v1_2_gdyn_d128_wideread_regsup_w0p1_newsampling_psuniform_landsat_refl.py"
)
