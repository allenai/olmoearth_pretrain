"""Text-embedding targets with original vec loss (no negative masking).

Same as text_targets.py but uses modality_patch_discrimination_vec instead of
masked_negatives. Tests whether text targets help even without masking.
"""
...
def build_train_module_config(common: CommonComponents):
    """Base speedup train module + text targets with original vec loss."""
