"""MRL for both token-level (patch disc) and instance-level (InfoNCE)."""
...
def build_train_module_config(
    ...
    return ContrastiveLatentMIMTrainModuleConfig(
        ...
        # MRL for token-level (patch discrimination)
        # Lower weights for smaller dims since patch disc is harder at low dims
        loss_config=LossConfig(
            loss_config={
                "type": "matryoshka_modality_patch_discrimination",
                "tau": 0.1,
                "dims": [48, 96, 192, 384, 768],
                "relative_weights": [0.25, 0.5, 0.75, 1.0, 1.0],
            }
        ),
        # MRL for instance-level (InfoNCE)
        # Instance contrastive is easier, can use uniform weights
        contrastive_config=LossConfig(
            loss_config={
                "type": "MatryoshkaInfoNCE",
                "weight": 0.1,
                "dims": [48, 96, 192, 384, 768],
                "relative_weights": [0.5, 0.75, 1.0, 1.0, 1.0],
            }
        ),
...
def build_trainer_config(common: CommonComponents) -> TrainerConfig:
    ...
    # keep the steps constant
    MAX_DURATION = Duration.epochs(300)
    METRICS_COLLECT_INTERVAL = 10
    CANCEL_CHECK_INTERVAL = 25
    LOAD_STRATEGY = LoadStrategy.if_available
    WANDB_USERNAME = "eai-ai2"  # nosec
    WANDB_PROJECT = "2026_01_15_mrl"
