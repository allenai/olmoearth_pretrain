...
def build_train_module_config(
    ...
    return ContrastiveLatentMIMTrainModuleConfig(
        ...
        # Token Merging (ToMe) configuration for encoder speedup
        # Merges similar tokens after specified layers to reduce computation
        tome_config=ToMeConfig(
            enabled=True,
            merge_layers=[3, 6, 9],  # Merge after blocks 3, 6, 9 (for 12-layer encoder)
            r_ratio=0.2,  # Reduce 20% of tokens per merge layer
        ),
    )
...
def build_trainer_config(common: CommonComponents) -> TrainerConfig:
    ...
    WANDB_PROJECT = "2026_02_03_token_merge"
