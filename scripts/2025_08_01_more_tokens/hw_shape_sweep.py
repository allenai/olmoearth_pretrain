...
for hw_list in HW_LIST:
    ...
    run_cmd = f"python train_cross_random_shape.py {run_name} {cluster} --launch.priority=high --launch.num_gpus=8 --data_loader.sampled_hw_p_list={hw_list}"
    print(run_cmd)
...
for hw_list in HW_LIST:
    ...
    run_cmd = f"python train_cross_random_shape.py {run_name} {cluster} --launch.priority=high --launch.num_gpus=8 --data_loader.sampled_hw_p_list={hw_list} --common.nccl_debug=True"
    print(run_cmd)
    subprocess.run(run_cmd, shell=True)  # nosec
    break
