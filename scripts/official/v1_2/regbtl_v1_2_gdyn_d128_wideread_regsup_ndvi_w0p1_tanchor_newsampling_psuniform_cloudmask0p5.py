"""cand_ndvi + cloud-aware patch discrimination, drop tokens over 50% cloud.
...
Swept arm: 0.25 / 0.5 / 0.75. This is the middle. A low threshold drops aggressively and
risks starving the loss of targets (``PatchDiscriminationLoss`` skips samples that end up
with zero decode tokens, and near-empty samples contribute a near-trivial loss that
dilutes the batch mean); a high one only removes the thickest cloud. Note the sampler
draws ps=1..8 uniformly and at ps=1 the per-token fraction is 0 or 1, so the threshold
...
MODULE_PATH = (
    ...
    "regbtl_v1_2_gdyn_d128_wideread_regsup_ndvi_w0p1_tanchor_newsampling_psuniform_cloudmask0p5.py"
)
