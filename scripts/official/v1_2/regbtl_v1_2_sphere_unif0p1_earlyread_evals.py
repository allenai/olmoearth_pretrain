"""The sphere + uniformity (unif0p1) run evaluated on the EARLY-READ six-task embedding set.

An eval-only shim. ``build_model_config`` and every data builder are imported unchanged
from ``regbtl_v1_2_gdyn_d128_wideread_regsup_ndvi_w0p1_tanchor_newsampling_psuniform_sphere_unif0p1``, so the architecture matches
...
pastis_ws16_ps1 bridge task), while the sphere_unif0p1 run runs the catalog. Only the
...
the sphere_unif0p1 run on those six tasks at whatever steps are swept, which also
...
from regbtl_v1_2_gdyn_d128_wideread_regsup_ndvi_w0p1_tanchor_newsampling_psuniform_sphere_unif0p1 import (  # noqa: F401
...
MODULE_PATH = "scripts/official/v1_2/regbtl_v1_2_sphere_unif0p1_earlyread_evals.py"
