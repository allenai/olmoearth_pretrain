"""EARLY READ: 6-layer trunk + 16 bottleneck blocks.

Replaces ``regbtl_v1_2_earlyread_e6_l25_d128_ndvi_tanchor_newsampling_psuniform``, which OOM'd on every attempt.
That arm was solved for exact parameter parity with the 12+4 base (121.89M) and needed
25 blocks to get there; at micro 64 it exceeded 80 GB and a rank died at step 6,
...
WHY 16 AND NOT 25: doubling the cheap arm's bottleneck depth (8 -> 16) keeps
the memory within budget at the SAME microbatch (a read block's saved activations are the
dominant scaling term, and 16 blocks is 0.64x the 25 that failed) while
closing most of the parameter gap: 103.53M, 0.85x the base, against the cheap arm's
0.72x.
...
instead is a SLOPE -- e6_l8 and e6_l16 share a trunk and differ only in
...
MODULE_PATH = (
    ...
    "regbtl_v1_2_earlyread_e6_l16_d128_ndvi_tanchor_newsampling_psuniform.py"
)


def build_model_config(common: CommonComponents) -> LatentMIMConfig:
    """6-layer trunk + 16 interleaved read/latent blocks, anchored reads, regsup+NDVI."""
