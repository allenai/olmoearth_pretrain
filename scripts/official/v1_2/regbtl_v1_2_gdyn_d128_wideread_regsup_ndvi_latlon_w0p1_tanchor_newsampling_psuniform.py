"""d128 wideread + regsup + NDVI + latlon (w0p1, newsampling, UNIFORM ps), anchored read.

``regbtl_v1_2_gdyn_d128_wideread_regsup_ndvi_latlon_w0p1_tanchor_newsampling`` with
``patch_size_probs`` reverted to uniform over ps=1..8. Every other newsampling knob,
``register_temporal_anchor="year_start"``, and both supervision arms (time-conditioned
NDVI + latlon regression, each a decode-only TARGET) are unchanged.

WHY: the full-supervision cell of the psuniform mirror. See
``regbtl_v1_2_gdyn_d128_wideread_regsup_ndvi_w0p1_tanchor_newsampling_psuniform`` for the
rationale -- every NDVI/tanchor/latlon verdict so far was measured under the
ps=1-oversampled sampler that the patch-size sweep showed to be a confound.
"""
...
MODULE_PATH = (
    ...
    "regbtl_v1_2_gdyn_d128_wideread_regsup_ndvi_latlon_w0p1_tanchor_newsampling_psuniform.py"
)
...
def build_dataloader_config(common: CommonComponents):
    """ndvi+latlon-aware newsampling dataloader at uniform patch sizes."""
...
def build_train_module_config(common: CommonComponents):
    """ndvi+latlon-aware faster train module with a halved rank microbatch size."""
