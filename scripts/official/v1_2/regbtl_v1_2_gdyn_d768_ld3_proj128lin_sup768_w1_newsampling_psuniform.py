"""RC recipe at register_latent_depth=3: price the bottleneck's block count.

THE CHANGE, AND ONLY IT: ``register_latent_depth = 3`` -- the interleaved
bottleneck runs 3 ``[read -> latent self-attention]`` block pairs instead of the
...
release candidate), so the pair is a one-flag depth contrast. The ld2 sibling
...
EVAL-ARM NAMING: the ``ld3`` token sits BEFORE ``proj128lin`` so no existing arm
name is a prefix of this one (the CSV export merges arms by longest prefix; a
trailing ``_ld3`` suffix would fold these rows into the RC's).
"""
...
MODULE_PATH = "scripts/official/v1_2/regbtl_v1_2_gdyn_d768_ld3_proj128lin_sup768_w1_newsampling_psuniform.py"

REGISTER_LATENT_DEPTH = 3


def build_model_config(common: CommonComponents) -> LatentMIMConfig:
    """The RC model with a 3-block ([read -> self] x 3) bottleneck."""
