"""d128 wideread + regsup + NDVI (w0p1) with the anchored register read, OLD sampling.

``regbtl_v1_2_gdyn_d128_il_pdproj_noic_lsa_wideread_regsup_w0p1_tanchor`` plus the
...
never a model input. NOTE: under this OLD sampling recipe ndvi rides along in the
batch and counts against the token budget (~one extra S2-bandset's worth); the
...
MODULE_PATH = (
    ...
    "regbtl_v1_2_gdyn_d128_il_pdproj_noic_lsa_wideread_regsup_ndvi_w0p1_tanchor.py"
)
...
def build_dataloader_config(common: CommonComponents):
    """1fwd dataloader whose masking knows ndvi is decode-only."""
    return build_extra_decode_dataloader_config(common, EXTRA_DECODE_MODALITIES)


def build_train_module_config(common: CommonComponents) -> LatentMIMTrainModuleConfig:
    """Faster (1fwd + fused AdamW + ddp/bf16) train module, ndvi decode-only."""
