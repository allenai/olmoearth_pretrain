"""d128 wideread + regsup + NDVI (psuniform, tanchor) with a GRAM-MATCHING pretext loss.
...
NOTE on scale: Gram MSE runs orders of magnitude smaller than the ~4-nat InfoNCE CE,
so the effective pretext:supervision gradient ratio shifts vs the baseline even
though AdamW is invariant to global loss scale. The regsup weights are left
untouched; revisit ``weight`` on the loss if supervision visibly dominates early.
"""
...
def build_train_module_config(common: CommonComponents):
    ...
    config.loss_config = LossConfig(loss_config={"type": "modality_gram_matching_vec"})
    return config
