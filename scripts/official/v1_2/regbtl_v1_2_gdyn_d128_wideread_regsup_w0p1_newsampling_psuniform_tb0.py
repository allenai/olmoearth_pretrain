"""d128 wideread + regsup (w0p1, newsampling, uniform ps) at temporal_bias 0.

``regbtl_v1_2_gdyn_d128_wideread_regsup_w0p1_newsampling_psuniform`` with the temporal
bias turned OFF: timesteps are drawn uniformly over their feasible window, the
historical behaviour. Budget stays at the committed 3072 and every other knob is
unchanged, so this is compute-neutral -- same tokens per step, same wall-clock, same
662,700-step schedule as the baseline.

WHY: this is the null anchor for the bias axis -- it answers whether ``temporal_bias``
is load-bearing at all. It matters because the newsampling gain is still unexplained.
The 4-point P(ps=1) sweep ruled out patch-size oversampling (flat 0.125 -> 0.70), and
budget 6144 vs 3072 accounts for only ~+0.005, so the temporal knobs are the last
untested part of the bundle.
...
MODULE_PATH = (
    ...
    "regbtl_v1_2_gdyn_d128_wideread_regsup_w0p1_newsampling_psuniform_tb0.py"
)
...
def build_dataloader_config(common: CommonComponents):
    """Uniform-patch-size newsampling dataloader at budget 3072, temporal_bias 0."""
