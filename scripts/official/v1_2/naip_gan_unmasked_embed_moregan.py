"""v1.2 NAIP unmasked-embedding conditional GAN with a stronger adversarial term.

Same as ``naip_gan_unmasked_embed.py`` (discriminator conditions on the online
encoder's pooled embedding from a full-depth forward on the unmasked input) but
rebalances the losses toward the GAN objective: the L1 reconstruction weight is
lowered (``lambda_l1=2.0``) and the adversarial weight is raised
(``lambda_adv=0.5``).

Validate before launching::

    python3 scripts/official/v1_2/naip_gan_unmasked_embed_moregan.py dry_run naip_gan_unmasked_embed_moregan local
"""
...
def build_train_module_config(common: CommonComponents) -> NaipGanTrainModuleConfig:
    """Build the unmasked-embedding train module config with a stronger GAN term."""
