"""v1.2 (worldcover_pred variant: use time-aligned predicted WorldCover.

This is a self-contained copy of ``scripts/official/v1_2/base.py``. The only
functional changes are:

* The ``worldcover`` modality is replaced with ``worldcover_pred`` in both
  ``training_modalities`` and ``ONLY_DECODE_MODALITIES``. ``worldcover_pred`` is
  the land-cover map predicted by the WorldCover segmentation model from each
  sample's own Sentinel-2 stack (see ``docs/Dataset-Creation.md``), so its time
  range reflects the sample year rather than a fixed 2021 map.
* The H5 dataset path points at the build that contains ``worldcover_pred``
  (and drops the unused ``gse`` / ``worldpop`` modalities).

Everything else (mixed 3D RoPE, hidden patch-embed projection) matches
v1_2/base.py.
"""
...
def build_dataset_config(common: CommonComponents) -> OlmoEarthDatasetConfig:
    ...
    return OlmoEarthDatasetConfig(
        h5py_dir="/weka/dfive-default/helios/dataset/osm_sampling/h5py_data_w_missing_timesteps_zstd_3_128_x_4/cdl_landsat_openstreetmap_raster_sentinel1_sentinel2_l2a_srtm_worldcereal_worldcover_pred_wri_canopy_height_map/1138828",
...
def build_trainer_config(common: CommonComponents) -> TrainerConfig:
    ...
    WANDB_PROJECT = "2026_07_v1_2_worldcover_pred"
