"""v1.2 register bottleneck noic_lsa d128 wideread + register supervision at 100x weight.

Identical to ``regbtl_v1_2_gdyn_d128_il_pdproj_noic_lsa_wideread_regsup.py`` except
the supervision base weight is raised 100x (0.01 -> 1.0); the per-task balancing
(``TASK_TYPE_WEIGHTS``) is unchanged, so effective weights become regression 1.0,
classification/BCE 0.1. Upper end of the supervision-weight sweep. Trains at the
base.py default 300 epochs.
...
MODULE_PATH = "scripts/official/v1_2/regbtl_v1_2_gdyn_d128_il_pdproj_noic_lsa_wideread_regsup_w1.py"


def build_model_config(common: CommonComponents) -> LatentMIMConfig:
    """d128 wideread + register-grid supervision at the 100x (1.0) base weight."""
