"""Sibling of the plain multistudent run, on the ndvi + tanchor recipe.

Identical arms, identical controls, identical evals -- the ONLY difference from
``regbtl_v1_2_gdyn_d768_multistudent_w1_newsampling_psuniform`` is the teacher
recipe: time-conditioned NDVI supervision plus a year-start anchored register read.
Launched as a pair with it, which is what makes the tanchor/ndvi question answerable
without confounding the student sweep: WITHIN each run every arm shares one teacher
and one read anchoring, so the arm comparisons stay clean; BETWEEN the two runs the
same five-arm profile is measured under both recipes, so a student-side conclusion
that holds under one and not the other shows up as a disagreement rather than
hiding inside a single run.

That pairing is also why tanchor is safe here at all. It re-anchors the perceiver
STUDENT's read as well as the teacher's (the student mirrors the primary's
``register_temporal_anchor``), so mixing anchored and unanchored arms inside one run
would confound every comparison. Holding it fixed per run avoids that entirely.
...
On top of that: ``include_ndvi`` adds the time-conditioned NDVI regression head (a
small MLP on ``[register_cell ; phi(day_of_year)]``, forcing each cell to store its
own decodable temporal trajectory) and ``temporal_anchor="year_start"`` anchors the
register read at Jan 1 of the sample's first observation year. NDVI rides along as a
supervision-only decode modality -- derived from the raw S2 B04/B08 in the dataset,
never a model input.
"""
...
MODULE_PATH = (
    ...
    "regbtl_v1_2_gdyn_d768_multistudent_ndvi_w1_tanchor_newsampling_psuniform.py"
)

# Anchor the register read at Jan 1 of the sample's first observation year, so the
# read's temporal coordinate is ~day-of-year: season-selective heads, year-invariant.
...
def build_dataloader_config(common: CommonComponents):
    """Newsampling dataloader whose masking knows ndvi is decode-only."""
