"""d128 wideread + regsup + NDVI + latlon (w0p1, newsampling), time-free register read.

``regbtl_v1_2_gdyn_d128_wideread_regsup_ndvi_w0p1_newsampling`` with the latlon
location-regression arm added (unit-sphere xyz from the mean-pooled register
grid; a supervised TARGET, never a model input). See that script and
``regbtl_v1_2_regsup_common`` for the NDVI time-conditioned head details.

Identical to
``regbtl_v1_2_gdyn_d128_wideread_regsup_ndvi_latlon_w0p1_tanchor_newsampling``
minus the temporally-anchored register read, isolating the NDVI+latlon arms from
the anchor.
"""
...
MODULE_PATH = (
    ...
    "regbtl_v1_2_gdyn_d128_wideread_regsup_ndvi_latlon_w0p1_newsampling.py"
)


def build_model_config(common: CommonComponents) -> LatentMIMConfig:
    """d128 wideread + regsup incl. NDVI + latlon heads, time-free register read."""
...
def build_dataloader_config(common: CommonComponents):
    """ndvi+latlon-aware dataloader with the decorrelated shape sampler applied."""
