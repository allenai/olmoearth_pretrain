"""v1.2 register bottleneck: gdyn + il + pdproj, instance contrastive ON, latent self-attn ON.

Builds the spatial register bottleneck on the v1.2 base (3D-RoPE encoder, 2D-RoPE
bottleneck + decoder -- see ``regbtl_v1_2_common``). This arm keeps the InfoNCE instance
contrastive loss from the v1.2 base (``ic``, projecting from the register latents) and
keeps the bottleneck's latent self-attention (``lsa``).
...
MODULE_PATH = "scripts/official/v1_2/regbtl_v1_2_gdyn_d768_il_pdproj_ic_lsa.py"
