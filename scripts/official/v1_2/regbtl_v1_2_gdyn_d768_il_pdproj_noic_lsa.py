"""v1.2 register bottleneck: gdyn + il + pdproj, instance contrastive OFF, latent self-attn ON.

Same as ``regbtl_v1_2_gdyn_d768_il_pdproj_ic_lsa`` but drops the InfoNCE instance
contrastive loss (``noic``: ``contrastive_config=None``), so the pretext is the JEPA /
patch-discrimination loss alone. Keeps the bottleneck's latent self-attention (``lsa``).
...
MODULE_PATH = "scripts/official/v1_2/regbtl_v1_2_gdyn_d768_il_pdproj_noic_lsa.py"
