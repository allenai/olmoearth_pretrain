r"""Launch script: osm_sampling + open-set supervised pretraining, d768 latent.
...
and contribute only the self-supervised (and map supervision) losses, while the
open-set samples add the supervised segmentation + regression signal.

The model is the register-bottleneck (Perceiver) ``wideread`` recipe with a
768-wide spatial latent grid; the open-set probe and the map supervision heads
both read that grid.

Usage (from the repo root)::

    python scripts/official/v1_2/open_set_osm_d768.py launch open_set_osm_d768 \\
...
MODULE_PATH = "scripts/official/v1_2/open_set_osm_d768.py"
