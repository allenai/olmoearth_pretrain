r"""Launch script: open-set mid-training on the d768 spatial latent.

Initializes from the finished ``regbtl_v1_2_gdyn_d768_regsup_w0p1`` pretraining
checkpoint (667,200 steps), trains ONLY the open-set probe for the first 40k
steps (backbone frozen), then unfreezes everything at a low backbone LR. Trains
on the full osm_sampling + open-set concat dataset throughout. See
...
load and is propagated to the Beaker job)::
    ...
    python scripts/official/v1_2/open_set_mid_d768.py launch open_set_mid_d768 \
...
MODULE_PATH = "scripts/official/v1_2/open_set_mid_d768.py"
...
def build_trainer_config(common: CommonComponents):
    """Trainer config: init from the d768 checkpoint, eval jobs point here."""
