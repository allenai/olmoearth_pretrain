...
# the budget) at token_budget 3072 (full year up to a 9x9 register grid) and the base
# rank_microbatch_size 64. All knobs live in regbtl_v1_2_newsampling_common.py; the
# architecture is baked into the scripts (not CLI overrides) so the Beaker eval jobs
# reconstruct the matching model.
set -e
...
# (the earlier OOM was largely 6+ GiB reserved-but-unallocated). Propagated to the
# Beaker job by internal/common.py.
...
python "$REGSUP" launch "regbtl_v1_2_gdyn_d128_wideread_regsup_w0p1_newsamp_v6" "$CLUSTER" \
    $LAUNCH_ARGS \
    --trainer.callbacks.wandb.project="$PROJECT"

python "$REGSUP_LATLON" launch "regbtl_v1_2_gdyn_d128_wideread_regsup_latlon_w0p1_newsamp_v6" "$CLUSTER" \
    $LAUNCH_ARGS \
    --trainer.callbacks.wandb.project="$PROJECT"
