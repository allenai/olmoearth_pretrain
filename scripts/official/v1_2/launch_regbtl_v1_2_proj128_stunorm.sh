#!/bin/bash
# Launch the stunorm base run: the RC lineage's (flat gram, linear head) cell.
#
# A one-flag mirror of lin_sup768_w1 -- register_projection_output_norm=True on the
# linear student, everything else byte-identical to its base -- with the in-loop eval
# set replaced by the AEF balanced trials + PASTIS on unmasked S1+S2+Landsat at BOTH
# student widths (d128 and d64). See the module's docstring.
#
# NOTE the Beaker job clones the repo and checks out $GIT_REF, so the code this run
# trains is whatever is COMMITTED AND PUSHED -- not the working tree. Commit and push
# before launching, or the run silently trains the unflagged architecture.
#
