"""v1.2 register bottleneck noic_lsa d128 wideread + register sup + latlon at 100x weight.

Identical to ``regbtl_v1_2_gdyn_d128_il_pdproj_noic_lsa_wideread_regsup_latlon.py``
except the supervision base weight is raised 100x (0.01 -> 1.0); the per-task balancing
(``TASK_TYPE_WEIGHTS``) is unchanged, so effective weights become regression 1.0
(incl. latlon), classification/BCE 0.1. Location stays a supervised TARGET, never a
model input. Upper end of the supervision-weight sweep. Trains at the base.py default
300 epochs.
...
MODULE_PATH = "scripts/official/v1_2/regbtl_v1_2_gdyn_d128_il_pdproj_noic_lsa_wideread_regsup_latlon_w1.py"


def build_model_config(common: CommonComponents) -> LatentMIMConfig:
    """d128 wideread + register supervision incl. latlon head, at the 100x (1.0) weight."""
