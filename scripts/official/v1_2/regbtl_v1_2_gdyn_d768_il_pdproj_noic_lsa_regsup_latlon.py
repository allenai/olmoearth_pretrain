"""v1.2 register bottleneck noic_lsa d768 + register-grid supervision + latlon target.

The ``regsup`` run (see ``regbtl_v1_2_gdyn_d768_il_pdproj_noic_lsa_regsup.py``) plus
location awareness as a SUPERVISED TARGET: the sample's (lat, lon) is regressed as
unit-sphere cartesian coordinates from the mean-pooled register grid, at the same low
regression weight (0.01). Location is never a model input -- latlon rides along in
the batch (dataset + only-decode masking overrides in ``regbtl_v1_2_regsup_common``)
but the encoder/decoder never tokenize it, so inference needs no coordinates and the
MIM objective can't shortcut through them.
...
def build_model_config(common: CommonComponents) -> LatentMIMConfig:
    """d768 frontier + register-grid supervision incl. the unit-sphere latlon head."""
