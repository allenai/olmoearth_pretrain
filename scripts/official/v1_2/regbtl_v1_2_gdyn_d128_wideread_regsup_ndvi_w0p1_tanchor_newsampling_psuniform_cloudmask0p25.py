"""cand_ndvi + cloud-aware patch discrimination, drop tokens over 25% cloud.
...
Swept arm: 0.25 / 0.5 / 0.75. This is the AGGRESSIVE end -- it drops a token once a
quarter of it is cloud, so it is the arm most at risk of starving the loss of targets
(``PatchDiscriminationLoss`` skips samples that end up with zero decode tokens, and
near-empty samples contribute a near-trivial loss that dilutes the batch mean). If the
sweep is non-monotonic, this arm is where to look first. Note the sampler draws ps=1..8
uniformly and at ps=1 the per-token fraction is 0 or 1, so the threshold
only discriminates at ps>=2.
...
CLOUD_SKIP_THRESHOLD = 0.25
MODULE_PATH = (
    "scripts/official/v1_2/"
    "regbtl_v1_2_gdyn_d128_wideread_regsup_ndvi_w0p1_tanchor_newsampling_psuniform_cloudmask0p25.py"
)
