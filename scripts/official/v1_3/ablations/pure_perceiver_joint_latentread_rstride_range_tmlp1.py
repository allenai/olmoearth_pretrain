"""Random-stride latentread (interval latents in time) with a light CoLT5-style token MLP.

``pure_perceiver_joint_latentread_rstride_range.py`` with ``token_mlp_ratio=1.0``: in every joint block the tokens get their
...
MACs of ``pure_perceiver_joint_latentread_rstride_range.py`` and ~0.60 s/step. Everything else is ``pure_perceiver_joint_latentread_rstride_range.py``.

W&B project ``20260921_perceiver_shapes``; trained as ``v1_3_vit0_rstride_ps8_tmlp1_latentread_rangerope_joint12``.
"""
...
MODULE_PATH = "scripts/official/v1_3/ablations/pure_perceiver_joint_latentread_rstride_range_tmlp1.py"
...
def build_model_config(common: CommonComponents) -> LatentMIMConfig:
    """``pure_perceiver_joint_latentread_rstride_range``'s model with a light token MLP (latents keep the full MLP)."""
