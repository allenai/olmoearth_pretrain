"""Random-stride latentread (interval latents in time and space) with a light CoLT5-style token MLP.

``pure_perceiver_joint_latentread_rstride_range_srange.py`` with ``token_mlp_ratio=1.0``: in every joint block the tokens get their
...
MACs of ``pure_perceiver_joint_latentread_rstride_range_srange.py`` and ~0.60 s/step. Everything else is ``pure_perceiver_joint_latentread_rstride_range_srange.py``.

W&B project ``20260921_perceiver_shapes``; trained as ``v1_3_vit0_rstride_ps8_tmlp1_srange_latentread_rangerope_joint12``.
"""
...
def build_model_config(common: CommonComponents) -> LatentMIMConfig:
    """``pure_perceiver_joint_latentread_rstride_range_srange``'s model with a light token MLP (latents keep the full MLP)."""
