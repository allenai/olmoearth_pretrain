"""Random-stride joint latentread with interval latents in time AND space.

``pure_perceiver_joint_latentread_rstride_range.py`` with ``latent_spatial_range=True``: every latent is encoded in RoPE as the
...
W&B project ``20260921_perceiver_shapes``; trained as ``v1_3_vit0_rstride_srange_latentread_rangerope_joint12``.
"""
...
MODULE_PATH = "scripts/official/v1_3/ablations/pure_perceiver_joint_latentread_rstride_range_srange.py"


def build_model_config(common: CommonComponents) -> LatentMIMConfig:
    """``pure_perceiver_joint_latentread_rstride_range``'s model with footprint-aware (spatially gated) latents."""
