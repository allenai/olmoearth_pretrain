"""Windowed-attention experiment: v1.3 with 5x5 neighborhood attention in the encoder.

THE CHANGE, AND ONLY IT: ``encoder_config.windowed_attention_size = 5``. Every
encoder self-attention block becomes neighborhood attention over the patch grid:
a spatial patch token attends only to tokens within a 5x5 patch neighborhood
(Chebyshev radius 2), across all timesteps, band sets, and spatial modalities.
...
See ``windowed_attention_3x3.py`` for the 3x3 arm.
...
MODULE_PATH = "scripts/official/v1_3/experiments/windowed_attention_5x5.py"
...
def build_model_config(common: CommonComponents) -> LatentMIMConfig:
    """The v1.3 model with 5x5 neighborhood attention in every encoder block."""
