"""hidden1 + 64-dim output bottleneck (AlphaEarth-style), with a configurable packer.

This is the ``hidden1`` recipe (single-bandset S2/Landsat, no-S1 random-time
masking, band dropout on S2/Landsat, masked-negatives patch discrimination +
InfoNCE) but the encoder now compresses every token to a 64-dim output
embedding via ``output_embedding_size``.

The thing we're ablating is the *packer*: the projection that maps the 768-dim
encoder tokens down to 64. By default that is a single ``nn.Linear`` (a hard
bottleneck). ``output_proj_hidden_sizes`` swaps it for a non-linear MLP
(Linear -> LayerNorm -> GELU -> ... -> Linear) so the model has more capacity
to pack information before the bottleneck.

Variant scripts in this directory import ``build_model_config_with`` and set
increasingly bigger ``output_proj_hidden_sizes``.

Adapted from scripts/official/v1_1/base.py (identical recipe; adds the bottleneck).
"""
...
# Per-pixel MLP applied BEFORE patchification (the hidden1 recipe).
PATCH_EMBED_HIDDEN_SIZES: list[int] = [64]
# Size of the bottlenecked output embedding (AlphaEarth uses 64).
...
def build_trainer_config(common: CommonComponents) -> TrainerConfig:
    ...
    WANDB_PROJECT = "2026_05_28_output_bottleneck_64"
...
def build_model_config_with(
    common: CommonComponents,
    output_proj_hidden_sizes: list[int] | None,
) -> LatentMIMConfig:
    """Build the model config with a 64-dim output bottleneck.

    Args:
        common: shared components (modalities, tokenization).
        output_proj_hidden_sizes: hidden widths for the encoder packer MLP. None
            (or empty) keeps the original single-linear bottleneck.
    """
    ...
    encoder_config = EncoderConfig(
        ...
        output_proj_hidden_sizes=output_proj_hidden_sizes,
    )
    decoder_config = PredictorConfig(
        # Decoder ingests the bottlenecked 64-dim encoder tokens.
        encoder_embedding_size=OUTPUT_EMBEDDING_SIZE,
...
def build_model_config(common: CommonComponents) -> LatentMIMConfig:
    """Default model config: 64-dim bottleneck with a single-linear packer (control)."""
    return build_model_config_with(common, output_proj_hidden_sizes=None)
