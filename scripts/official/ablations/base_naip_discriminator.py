"""Contrastive Latent MIM with adversarial discriminator loss on NAIP_10.

Extends the base phase-2 config by:
  1. Adding NAIP_10 to the training modalities.
  2. Using the osm_sampling dataset that includes NAIP_10.
  3. Training a small token-level discriminator that distinguishes "real" NAIP tokens
     (from the EMA target encoder) from "fake" NAIP tokens (from the decoder).
  4. Adding an adversarial generator loss so the decoder is encouraged to produce
     NAIP token representations indistinguishable from the target encoder's.
"""
...
from olmoearth_pretrain.train.train_module.contrastive_latentmim_discriminator import (
    DiscriminatorContrastiveLatentMIMTrainModuleConfig,
)
...
def build_train_module_config(
    ...
    model_size = MODEL_SIZE_ARGS["base_shallow_decoder"]
    decoder_dim = model_size["decoder_embedding_size"]

    return DiscriminatorContrastiveLatentMIMTrainModuleConfig(
...
def build_dataset_config(common: CommonComponents) -> OlmoEarthDatasetConfig:
    ...
    return OlmoEarthDatasetConfig(
        h5py_dir=(
            ...
            "era5_10_landsat_naip_10_openstreetmap_raster_sentinel1_sentinel2_l2a_srtm_worldcover/"
            "1138828"
        ),
