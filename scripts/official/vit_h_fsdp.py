"""ViT-H recipe with 2-node FSDP.

This recipe uses a ViT-H-style encoder with a shallow decoder and defaults to a
2-node launch with standard FSDP across all GPUs:
...
Launch example:
    python scripts/official/vit_h_fsdp.py launch vit_h_fsdp ai2/jupiter \
        --launch.num_nodes=2 \
        --launch.num_gpus=8 \
        --launch.clusters=[ai2/jupiter,ai2/ceres] \
        --trainer.callbacks.wandb.project=YYYY_MM_DD_vith_fsdp
"""
...
def build_train_module_config(common: CommonComponents):
    """Build the train module config with FSDP + speed knobs."""
