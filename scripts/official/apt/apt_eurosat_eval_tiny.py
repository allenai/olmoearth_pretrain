...
This script loads a pretrained tiny model and evaluates on EuroSAT.

NOTE: APT (Adaptive Patch Transformers) is NOT yet integrated into the eval pipeline.
The APTConfig is defined here as a placeholder for future integration.
Currently uses standard uniform patching.

TODO: To integrate APT:
  1. Add APT partitioner to dataloader (precompute patch descriptors)
  2. Replace FlexiPatchEmbed with AdaptivePatchEmbed in model
  3. Add apt_config to DownstreamTaskConfig and wire through EvalWrapper
...
def build_trainer_config(common: CommonComponents) -> TrainerConfig:
    ...
    EVAL_TASKS = {
        "m-eurosat-finetune-apt": DownstreamTaskConfig(
            dataset="m-eurosat",
...
if __name__ == "__main__":
    # APT config is built but NOT YET USED in the eval pipeline
    # This is a placeholder for future APT integration
    apt_config = build_apt_config()
    logger.info(f"APT Configuration (NOT USED YET): {apt_config}")
    logger.info("Using TINY model configuration")
    logger.warning(
        "APT is NOT integrated into eval yet - using standard uniform patching. "
        "See docstring for integration TODOs."
    )
