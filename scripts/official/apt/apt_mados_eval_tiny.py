"""MADOS finetuning evaluation with tiny model.

This script loads a pretrained tiny model and evaluates on MADOS with APT
(Adaptive Patch Transformers) for adaptive patching based on image complexity.

Usage:
    # Local evaluation with the tiny checkpoint
    python scripts/official/apt/apt_mados_eval_tiny.py evaluate \
        apt_mados_eval_tiny local \
        --trainer.load_path=/weka/dfive-default/helios/checkpoints/joer/tiny_lr0.0002_wd0.02/step360000

    # On Beaker
    python scripts/official/apt/apt_mados_eval_tiny.py launch \
        apt_mados_eval_tiny ai2/saturn-cirrascale \
        --trainer.load_path=/weka/dfive-default/helios/checkpoints/joer/tiny_lr0.0002_wd0.02/step360000
    ...
    python scripts/official/apt/apt_mados_eval_tiny.py evaluate \
        ...
        --trainer.callbacks.downstream_evaluator.tasks.mados_finetune_apt.apt_config.partitioner.thresholds=[0.5]

    # Override APT num_scales (number of patch size scales: 1=base only, 2=base+2x, etc.)
    python scripts/official/apt/apt_mados_eval_tiny.py evaluate \
        apt_mados_eval_tiny local \
        --trainer.load_path=/path/to/checkpoint \
        --trainer.callbacks.downstream_evaluator.tasks.mados_finetune_apt.apt_config.partitioner.num_scales=3 \
        --trainer.callbacks.downstream_evaluator.tasks.mados_finetune_apt.apt_config.partitioner.thresholds=[0.5,1.0]

    # Override APT base_patch_size
    python scripts/official/apt/apt_mados_eval_tiny.py evaluate \
        apt_mados_eval_tiny local \
        --trainer.load_path=/path/to/checkpoint \
        --trainer.callbacks.downstream_evaluator.tasks.mados_finetune_apt.apt_config.partitioner.base_patch_size=2
...
def build_trainer_config(common: CommonComponents) -> TrainerConfig:
    ...
    # MADOS finetuning evaluation with APT
...
if __name__ == "__main__":
    logger.info("Using TINY model configuration with APT enabled for MADOS")
