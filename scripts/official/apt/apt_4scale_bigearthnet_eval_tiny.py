"""BigEarthNet finetuning with 4-scale APT (patch sizes 1, 2, 4, 8).

Tests whether adaptive patching across 4 scales can beat uniform patch-4
by using fewer tokens on average. Uses avg-init conv downsampling.

Usage:
    python scripts/official/apt/apt_4scale_bigearthnet_eval_tiny.py evaluate \
        apt_4scale_bigearthnet local \
        --trainer.load_path=/weka/dfive-default/helios/checkpoints/joer/tiny_lr0.0002_wd0.02/step360000

    python scripts/official/apt/apt_4scale_bigearthnet_eval_tiny.py launch \
        apt_4scale_bigearthnet ai2/saturn-cirrascale \
        --trainer.load_path=/weka/dfive-default/helios/checkpoints/joer/tiny_lr0.0002_wd0.02/step360000

    # Override thresholds (3 values for 4 scales: 2->1, 4->2, 8->4)
    --trainer.callbacks.downstream_evaluator.tasks.m_bigearthnet_finetune.apt_config.partitioner.thresholds=[0.5,0.8,1.2]
"""
...
if __name__ == "__main__":
    logger.info("4-scale APT (1,2,4,8) on BigEarthNet with avg-init")
    main(
