"""BigEarthNet finetuning evaluation with tiny model (no APT baseline).

This script loads a pretrained tiny model and evaluates on BigEarthNet
using standard uniform patching (no APT).

Usage:
    # Local evaluation with the tiny checkpoint
    python scripts/official/apt/bigearthnet_eval_tiny.py evaluate \
        bigearthnet_eval_tiny local \
    ...
    python scripts/official/apt/bigearthnet_eval_tiny.py evaluate \
        ...
        --trainer.callbacks.downstream_evaluator.tasks.bigearthnet_finetune.patch_size=8
"""
...
if __name__ == "__main__":
    logger.info("Using TINY model configuration (no APT) for BigEarthNet")
