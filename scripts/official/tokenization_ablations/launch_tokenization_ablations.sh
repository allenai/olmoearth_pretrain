...
python3 scripts/official/tokenization_ablations/base_single_band_s2.py launch base_single_band_s2_2 ai2/jupiter \
...
python3 scripts/official/tokenization_ablations/base_spectral_grouping.py launch base_spectral_grouping_2 ai2/jupiter \
  --launch.num_gpus=${NUM_GPUS} \
  --launch.clusters="${CLUSTERS}" \
  --launch.priority=${PRIORITY} \
  --trainer.callbacks.wandb.project=${WANDB_PROJECT}
...
python3 scripts/official/tokenization_ablations/base_all_bands_single_token.py launch base_all_bands_single_token_2 ai2/jupiter \
  --launch.num_gpus=${NUM_GPUS} \
  --launch.clusters="${CLUSTERS}" \
  --launch.priority=${PRIORITY} \
  --trainer.callbacks.wandb.project=${WANDB_PROJECT}
