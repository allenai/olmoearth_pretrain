...
if __name__ == "__main__":
        ...
        ExperimentSpec(
            ...
            tasks=[
                TaskSpec(
                    ...
                    command=["python", "helios/inference_benchmarking/run_throughput_benchmark.py"],
                    env_vars=[
                        EnvVar(name=key, value=value)
                        for key, value in config.to_env_vars().items()
                    ] + [
                        EnvVar(name=constants.PARAM_KEYS["project"], value=constants.PROJECT_NAME),
                        EnvVar(name=constants.PARAM_KEYS["owner"], value=constants.ENTITY_NAME),
                        EnvVar(name="WANDB_API_KEY", secret=f"{account.name}_WANDB_API_KEY")
                    ],
    ...
    for experiment_spec in experiment_specs:
        experiment_data = b.experiment.create(spec=experiment_spec, workspace=constants.BEAKER_WORKSPACE)
