"""Single bandset + random band dropout (no S1) + random time + eurocrops.

Based on base_band_dropout_no_s1_drop_random_time.py but trains on
osm_sampling + eurocrops datasets via concat dataset config, and adds
eurocrops as a decode-only modality.
"""
...
def build_dataset_config(common: CommonComponents) -> OlmoEarthConcatDatasetConfig:
    ...
    dataset_configs = [
        OlmoEarthDatasetConfig(
            h5py_dir="/weka/dfive-default/helios/dataset/osm_sampling/h5py_data_w_missing_timesteps_zstd_3_128_x_4/cdl_era5l_day_10_landsat_openstreetmap_raster_sentinel1_sentinel2_l2a_srtm_worldcereal_worldcover_wri_canopy_height_map/1138828",
