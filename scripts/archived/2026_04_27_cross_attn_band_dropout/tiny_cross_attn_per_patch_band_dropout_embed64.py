"""Cross-attention per-patch band dropout (embedding size 64) + single bandset + random time with decode masking + masked-negatives loss.

- Single bandset S2 (all 12 bands) / Landsat (all 11 bands)
- Cross-attention band dropout (rate ~ Uniform(0, 0.2)) on S2 and Landsat only (no S1 dropout)
- Per-patch band dropout: different bands dropped at each spatial patch (per_patch_band_dropout=True)
- Cross-attention embedding size: 64
...
def build_trainer_config(common: CommonComponents) -> TrainerConfig:
    ...
    WANDB_PROJECT = "2026_04_27_cross_attn_band_dropout"
