"""Memory leak bisect: drop only the Beaker callback.

Copy of hidden1_no_evals.py with the BeakerCallback removed entirely.
W&B and Checkpointer remain enabled. If the leak disappears here but
reappears when only W&B or only Checkpointer is dropped, the source is
Beaker (most likely the periodic Beaker API calls in post_step / the
beaker-py client).
"""
