"""Memory leak bisect: drop W&B, Beaker, and Checkpointer callbacks.

Copy of hidden1_no_evals_no_gc_callback.py with the three most
suspect callback sources disabled/removed:

- WandB: set enabled=False (kept in dict so experiment.py train() doesn't crash
  on ``trainer.callbacks["wandb"]``).
- Beaker: removed entirely.
- CheckpointerCallback: removed entirely (no checkpoint saves).

If the leak disappears with this config, the source is one of those three.
"""
...
def build_trainer_config(common: CommonComponents) -> TrainerConfig:
    ...
    # Disabled — no W&B logging, but kept in dict so train() doesn't KeyError.
