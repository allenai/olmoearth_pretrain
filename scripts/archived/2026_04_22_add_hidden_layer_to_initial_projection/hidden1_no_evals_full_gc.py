"""Add hidden layer to the initial projection layer — full gc variant.

Copy of hidden1_no_evals.py that swaps the GarbageCollectorCallback for one
that runs gc.collect() (all generations) instead of gc.collect(1) (gen 0+1
only). Tests the hypothesis that the per-epoch memory leak is caused by
cyclic DataLoader garbage being promoted to gen-2 and never collected.
"""
...
class FullGarbageCollectorCallback(GarbageCollectorCallback):
    """GC callback that runs full gc.collect() (all generations) every interval.

    The base class only runs gc.collect(1) which never reclaims gen-2 cyclic
    garbage like abandoned torch DataLoader iterators (whose pin_memory_thread
    forms a reference cycle with the iterator). After ~2 collections those
    objects are promoted to gen-2 and become permanently unreclaimable while
    auto-GC is disabled, leaking their workers and shared memory.
    """

    def post_step(self) -> None:
        """Run a full collection at the configured interval."""
        if not self.enabled:
            return
    ...
    def post_checkpoint_saved(self, path) -> None:  # type: ignore[override]
        """Run a full collection after saving a checkpoint."""
        del path
        if not self.enabled:
            return
        gc.collect()
...
def build_trainer_config(common: CommonComponents) -> TrainerConfig:
    ...
    WANDB_PROJECT = "2026_05_14_hidden1_no_evals_memleak_test"
    ...
    garbage_collector_callback = FullGarbageCollectorCallback(gc_interval=1)
    trainer_config = (
        TrainerConfig(
            work_dir=common.save_folder,
