"""Add hidden layer to the initial projection layer — no GC callback variant.

Copy of hidden1_no_evals.py that disables the GarbageCollectorCallback so
Python's automatic cyclic GC stays enabled. Tests the hypothesis that the
per-epoch memory leak is caused by the callback disabling auto-GC and only
running gc.collect(1), which never reclaims gen-2 cyclic garbage like
abandoned torch DataLoader iterators.

Note: We pass enabled=False rather than omitting the callback, because the
Trainer auto-adds a default GarbageCollectorCallback() if none is present.
"""
...
def build_trainer_config(common: CommonComponents) -> TrainerConfig:
    ...
    # enabled=False so the callback is a no-op (doesn't disable auto-GC and
    # doesn't run gc.collect(1)). Registering it under the same name as the
    # default also blocks the Trainer from auto-adding the default one.
