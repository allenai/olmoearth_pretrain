"""Memory leak bisect: drop only the Checkpointer callback.

Copy of hidden1_no_evals.py with the CheckpointerCallback removed entirely
(no checkpoint saves). W&B and Beaker remain enabled. If the leak
disappears here but reappears when only W&B or only Beaker is dropped,
the source is the checkpointer (most likely the async DCP save staging
state dicts on CPU and/or the Gloo bookkeeping process group used during
async checkpointing).
"""
