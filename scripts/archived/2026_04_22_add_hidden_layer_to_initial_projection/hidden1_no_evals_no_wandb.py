"""Memory leak bisect: drop only the W&B callback.

Copy of hidden1_no_evals.py with W&B disabled (kept in callback dict so
experiment.py train() doesn't KeyError on ``trainer.callbacks["wandb"]``,
but enabled=False so it does no work).

Beaker and Checkpointer callbacks remain enabled. If the leak disappears
here but reappears when only Beaker or only Checkpointer is dropped,
the source is W&B.
"""
...
def build_trainer_config(common: CommonComponents) -> TrainerConfig:
    ...
    # W&B disabled but still in callback dict — experiment.py's train()
    # accesses ``trainer.callbacks["wandb"]`` directly and would KeyError otherwise.
