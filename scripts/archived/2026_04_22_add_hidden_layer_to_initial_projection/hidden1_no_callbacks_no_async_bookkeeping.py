"""Memory leak bisect: drop callbacks + disable async bookkeeping.

Same callback changes as hidden1_no_callbacks.py (W&B disabled, Beaker and
Checkpointer removed), plus async_bookkeeping=False on the TrainerConfig.

This disables the separate Gloo process group the trainer creates for
async bookkeeping. If Gloo's CPU-tensor shared-memory transport is leaking
temp buffers on every all_reduce_value call, disabling this will stop the
leak.
"""
