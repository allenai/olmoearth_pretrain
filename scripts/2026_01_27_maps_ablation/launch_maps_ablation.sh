...
python scripts/official/base.py launch ablation_no_worldcover $CLUSTER \
    ...
    --train_module.masking_config.strategy_config.only_decode_modalities='[srtm,openstreetmap_raster,wri_canopy_height_map,cdl,worldcereal]' \
...
python scripts/official/base.py launch ablation_no_srtm $CLUSTER \
    ...
    --train_module.masking_config.strategy_config.only_decode_modalities='[worldcover,openstreetmap_raster,wri_canopy_height_map,cdl,worldcereal]' \
    --launch.clusters="$CLUSTERS" \
    --launch.priority=$PRIORITY \
    --trainer.callbacks.wandb.project=$WANDB_PROJECT
...
python scripts/official/base.py launch ablation_no_openstreetmap_raster $CLUSTER \
    ...
    --train_module.masking_config.strategy_config.only_decode_modalities='[worldcover,srtm,wri_canopy_height_map,cdl,worldcereal]' \
    --launch.clusters="$CLUSTERS" \
    --launch.priority=$PRIORITY \
    --trainer.callbacks.wandb.project=$WANDB_PROJECT
...
python scripts/official/base.py launch ablation_no_wri_canopy_height_map $CLUSTER \
    ...
    --train_module.masking_config.strategy_config.only_decode_modalities='[worldcover,srtm,openstreetmap_raster,cdl,worldcereal]' \
    --launch.clusters="$CLUSTERS" \
    --launch.priority=$PRIORITY \
    --trainer.callbacks.wandb.project=$WANDB_PROJECT
...
python scripts/official/base.py launch ablation_no_cdl $CLUSTER \
    ...
    --train_module.masking_config.strategy_config.only_decode_modalities='[worldcover,srtm,openstreetmap_raster,wri_canopy_height_map,worldcereal]' \
    --launch.clusters="$CLUSTERS" \
    --launch.priority=$PRIORITY \
    --trainer.callbacks.wandb.project=$WANDB_PROJECT
...
python scripts/official/base.py launch ablation_no_worldcereal $CLUSTER \
    ...
    --train_module.masking_config.strategy_config.only_decode_modalities='[worldcover,srtm,openstreetmap_raster,wri_canopy_height_map,cdl]' \
    --launch.clusters="$CLUSTERS" \
    --launch.priority=$PRIORITY \
    --trainer.callbacks.wandb.project=$WANDB_PROJECT
